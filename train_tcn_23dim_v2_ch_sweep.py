#!/usr/bin/env python3
"""TCN+SE 23-dim + B=64 + LR=4e-3 + ep=20 (3 轮 sweep 累积最佳) × Channels 扫描 + 5-seed

固定超参:
  - 23-dim = 27-dim - {length(2), setpoint(3), crc_mean_w(20), cmd_count_w(22)}
  - BATCH = 64 (batch sweep 甜蜜点)
  - LR = 4e-3 (LR sweep 甜蜜点)
  - EPOCHS = 20 (epoch sweep 甜蜜点)
  - WD = 1e-5, Dropout = 0.3, patience = 5
  - k=3, dilations=[1,2,4], SE r=8, W=16

扫描 channels ∈ {16, 32, 48, 64, 96}  × 5 seeds = 25 runs

历史依据:
  - [[project-tcn-v4-ch-sweep-full]] 19-dim 扫 ch=8-128,ch=48 PR-AUC 0.9329 冠军,ch=128 F1m 0.8729 冠军
  - 本工作新配置 (23-dim + B=64 + LR=4e-3) 甜蜜点可能左移到 ch=32 或 48

用法 (单跑):
    python train_tcn_23dim_v2_ch_sweep.py --channels 48 --seed 42
"""
...
# 5 个 channels 档 (从 MCU 级 16 到 大模型 96,覆盖 6x 范围)
...
def run_one(channels: int, seed: int):
...
def main():
    ...
    parser.add_argument("--channels", type=int, required=True, choices=CHANNELS_GRID)
    parser.add_argument("--seed", type=int, required=True, choices=SEEDS)
    parser.add_argument("--out", type=str, default=None)
    args = parser.parse_args()

    if args.out is None:
        args.out = os.path.join(BASE, f"train_tcn_23dim_v2_ch_sweep_partial_ch{args.channels}_s{args.seed}.json")

    print(f"[run] channels={args.channels}  seed={args.seed}", flush=True)
    t0 = time.time()
    r = run_one(args.channels, args.seed)
    print(f"[done] F1m={r['test_macro_f1']:.4f}  PR-AUC={r['test_pr_auc']:.4f}  "
          f"BestEp={r['best_epoch']}/{r['actual_epochs_run']}  "
          f"Params={r['n_params']:,}  TrainT={r['train_time_s']:.1f}s  WallT={time.time()-t0:.1f}s", flush=True)
