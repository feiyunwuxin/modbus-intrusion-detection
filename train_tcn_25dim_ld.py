#!/usr/bin/env python3
"""25 维 = 27 - {length (idx 2), deadband (idx 6)} retrain 5-seed 验证

连续剪裁路径: 27 -> 26 (-length) -> 25 (-length, -deadband)

5-seed LOO 单点预测:
  - length (idx 2):   DeltaF1m = +0.0051 (正向)
  - deadband (idx 6): DeltaF1m = -0.0043 (负向)

LOO 加总预测: +0.0051 - 0.0043 = +0.0008 (几乎中性)
期望 F1m >= 0.8205 + 0.0008 = 0.8213
...
# 25 维 = 27 - {2 length, 6 deadband}
KEEP_25 = [i for i in range(27) if i not in (2, 6)]
DROP = {2: "length", 6: "deadband"}
...
print(f"=== TCN+SE 25-dim (-length, -deadband) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
...
print("\n" + "="*80)
print("=== 25-dim (-length, -deadband) 5-seed 汇总 ===")
print("="*80)
...
print(f"  25-dim mean:        F1m = {mean_f1m:.4f}  DeltaF1m = {mean_f1m - baseline_f1m_5seed:+.4f}")
print(f"  25-dim seed=42:     F1m = {results[0]['test_macro_f1']:.4f}  DeltaF1m = {results[0]['test_macro_f1'] - baseline_f1m_5seed:+.4f}")
print(f"  LOO 加总预测:        DeltaF1m >= +0.0008 (= +0.0051 - 0.0043)")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0008 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0008 else 'NO FAIL'}")
...
print(f"  25-dim mean:        DeltaF1m = {mean_f1m - single_champion_f1m:+.4f}")
...
out_json = os.path.join(BASE, "train_tcn_25dim_ld_results.json")
with open(out_json, "w") as f:
    json.dump({"results_25dim": results,
