#!/usr/bin/env python3
"""
Ensemble v4 — Stack ALL_10: v3's 9 + 1D Transformer (new attention architecture).

Adds 1D Transformer Encoder (d_model=128, n_heads=4, n_layers=3, 605K params)
to the v3 lineup. Tests whether pure self-attention brings complementary
signal beyond LSTM+Attention and TCN families.

Compared to v3 (best F1m=0.8458, best PR-AUC=0.8541):
  - 10 base models
  - Tests ALL_10 and key ablations (drop-TF, drop-TCN_v3c, etc.)
  - Same uniform/weighted/LR-stacking protocol
"""
...
# 10 base models. v3 had 9, v4 adds 1D Transformer.
MODELS = [
    ...
    # NEW: 1D Transformer (pure self-attention, on v2 data)
    ("Transformer",  "predictions_val_transformer_v2_window16.csv",       "predictions_test_transformer_v2_window16.csv",       "w16",  16),
]
...
def load_pair(val_fn, test_fn, kind, win):
    ...
    v = pd.read_csv(vp); t = pd.read_csv(tp)
    if kind in ("w8", "w16"):
...
for name in val_dfs:
    ...
    assert (test_dfs[name]["y_true"].values == y_test_ref).all()
...
for name in val_dfs:
    ...
    r = indiv_results[-1]
    log(f"   {name:14s}  thr={r['best_thr']:.2f}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}  Bin-F1={r['test_f1b']:.4f}  Acc={r['test_acc']:.4f}")
...
log(f"   test shape: {test_probs.shape}")

# ─────────────────────────────────────────────
# Subsets to try
# ─────────────────────────────────────────────
SUBSETS = [
    # v2 baseline (7 models)
    ("ALL_7_v2",                ["LGB v2", "RF", "TCN v4", "TCN v3a", "TCN v3b", "TCN v3c", "LSTM+Att"]),
    # v3 baseline (9 models)
    ("ALL_9_v3",                ["LGB v2", "RF", "TCN v4", "TCN v3a", "TCN v3b", "TCN v3c", "LSTM+Att", "TCN v4 v2", "LSTM+Att v2"]),
    # v4 NEW — full 10
    ("ALL_10",                  names),
    # v4 NEW — Transformer加入 v2 (clean ALL_8)
    ("ALL_8_v4",                ["LGB v2", "RF", "TCN v4", "TCN v3a", "TCN v3b", "TCN v3c", "LSTM+Att", "Transformer"]),
    # Ablation: drop TCN v3c (which has negative coefficient in v2 ensemble)
    ("ALL_10_no_v3c",           ["LGB v2", "RF", "TCN v4", "TCN v3a", "TCN v3b", "LSTM+Att", "TCN v4 v2", "LSTM+Att v2", "Transformer"]),
    # Ablation: drop TCN v3a (lowest performer of TCN family)
    ("ALL_10_no_v3a",           ["LGB v2", "RF", "TCN v4", "TCN v3b", "TCN v3c", "LSTM+Att", "TCN v4 v2", "LSTM+Att v2", "Transformer"]),
    # Ablation: keep diverse — LGB+RF + 1 of each family + Transformer
    ("diverse_6",               ["LGB v2", "RF", "TCN v4", "TCN v3c", "LSTM+Att", "Transformer"]),
    # Ablation: attention-only + trees
    ("attn_3_plus_trees",       ["LGB v2", "RF", "LSTM+Att", "LSTM+Att v2", "Transformer"]),
    # Compare: just Transformer alone
    ("TF_alone",                ["Transformer"]),
]
...
for label, members in SUBSETS:
    ...
    r = uniform_results[-1]
    log(f"   {label:18s}  thr={r['best_thr']:.2f}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}")
...
for label, members in SUBSETS:
    ...
    log(f"   {label:18s}  weights={w.round(2).tolist()}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}")

# ─────────────────────────────────────────────
log("STEP 6: stacking with Logistic Regression ...")
stack_results = []
for label, members in SUBSETS:
    idx = [names.index(m) for m in members]
    ...
    r = stack_results[-1]
    log(f"   {label:18s}  C={best_C}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}  coefs={list(r['coefs'].values())}")

# ─────────────────────────────────────────────
log("STEP 7: save artifacts ...")
OUT  = os.path.join(BASE, "evaluation_ensemble_v4.txt")
META = os.path.join(BASE, "processed_meta_ensemble_v4.json")

lines = [
    "=" * 80, "ENSEMBLE REPORT — v4: ALL_10 (v3's 9 + 1D Transformer)",
    "=" * 80,
    "Base models:", ""
]
...
for sec, runs in [("METHOD 1 — UNIFORM AVERAGE", uniform_results),
    ...
    for r in runs:
        lines.append(f"  [{r['members']}]  thr={r['best_thr']:.2f}  "
                     f"F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}")
        if "weights" in r:
