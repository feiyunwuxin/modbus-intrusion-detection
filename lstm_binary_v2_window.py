#!/usr/bin/env python3
"""
LSTM binary classifier on the 17-feature v2 dataset, sliding window of 4 rows
...
- LSTM over the 4 timesteps, use final hidden state
- Val-based threshold tuning (no test leak)
- Compare against LGB v2, single-row CNN v2, window=4 CNN v2
"""
...
MODEL_OUT = os.path.join(BASE, "model_lstm_binary_v2_window.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_lstm_binary_v2_window.txt")
META_OUT  = os.path.join(BASE, "processed_meta_lstm_binary_window.json")
THR_OUT   = os.path.join(BASE, "best_threshold_lstm_window.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_lstm_binary_v2_window.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_lstm_binary_v2_window.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_lstm_binary_v2_window.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_lstm_binary_v2_window.png")
HIST_PNG  = os.path.join(BASE, "training_history_lstm_binary_v2_window.png")
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — LSTM Binary (v2, sliding window=4)")
lines.append("=" * 72)
