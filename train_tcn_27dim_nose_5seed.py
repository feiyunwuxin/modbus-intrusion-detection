#!/usr/bin/env python3
"""27 维 TCN 27-dim × {SE vs NoSE} × 5-seed 公平消融

完全复用 train_tcn_27dim_5seed.py 的超参,只切换 use_se=True/False
- 数据: SCADA v2 27-dim, window=16
- 模型: TCN(3 blocks, ch=64, k=3, dilations=[1,2,4])
- 超参: EPOCHS=20, BATCH=512, LR=5e-4, WD=1e-5, patience=5, dropout=0.3
- Seeds: [42, 123, 456, 789, 1024]
- 训练: BCEWithLogitsLoss + pos_weight, ReduceLROnPlateau
"""
...
def run_variant(label, use_se):
    ...
    results = []
    for seed in SEEDS:
        r = run_one(KEEP_27, seed=seed, use_se=use_se)
...
if __name__ == "__main__":
    print(f"=== TCN 27-dim × SE/NoSE 5-seed 公平消融 ===")
    print(f"WINDOW=16, blocks=3, ch=64, dilations=[1,2,4], LR=5e-4, WD=1e-5, dropout=0.3")
    print(f"Seeds: {SEEDS}")

    res_se    = run_variant("27-dim WITH SE",   use_se=True)
    res_nose  = run_variant("27-dim WITHOUT SE", use_se=False)

    sum_se   = summarize("27-dim WITH SE",    res_se)
    sum_nose = summarize("27-dim WITHOUT SE", res_nose)
    ...
    out = {
        "config": {
            "data": "27-dim SCADA v2, window=16",
    ...
    out_path = os.path.join(BASE, "train_tcn_27dim_nose_5seed_results.json")
