#!/usr/bin/env python3
"""CNN-LSTM hybrid (conv [256,256], LSTM hidden=128) on 23-dim × window=16, 5 seeds.

v2 scaled-up version of train_cnnlstm_23dim_w16_5seed.py:
  - conv channels [128, 128] → [256, 256]
  - LSTM hidden 64 → 128
  - params ~162K → ~610K
"""
import os
...
class CNNLSTMClassifier(nn.Module):
    ...
    def forward(self, x):
        # x: (B, 23, 16)
        x = F.relu(self.bn1(self.conv1(x)))  # (B, 256, 16)
        x = F.relu(self.bn2(self.conv2(x)))  # (B, 256, 16)
        # transpose to (B, 16, 256) for LSTM
        x = x.transpose(1, 2)
        out, _ = self.lstm(x)
...
def main():
    print(f"=== CNN-LSTM (conv=[256,256], LSTM hidden=128) 23-dim × window=16 × 5 seeds ===")
    results = []
    for seed in SEEDS:
        print(f"\n--- seed={seed} ---")
    ...
    save_meta_json(results, os.path.join(BASE_PATH, f"processed_meta_{TAG}.json"),
                   model_name="CNN-LSTM (ch256,h128)", tag=TAG,
