/* ============================================================================
 * main_stm32f407.c  —  STM32F407VGT6 TCN+SE ch=32 入侵检测主程序
 * ============================================================================
 *
 ...
 *
 * 时序:
 *   - 启动 ~3 ms  init dequant
 *   - 推理 ~0.9 ms 一次 (FP32 conv, Cortex-M4 168MHz, -O2)
 *   - 整个 SCADA Modbus 周期 (16 行 ≈ 1.6 秒) 推理一次
 *
 * 功能:
 *   1. 启动时打印版本 + Flash/RAM 信息
 *   2. 模拟一个 19 维 SCADA 窗口 (生产时从 Modbus UART 收)
 *   3. 每 16 行 (一个窗口) 做一次 ch32_forward
 *   4. p > 0.49 报警 (LED + 蜂鸣器 + 串口打印)
 ...
 *   - 打开 FPU (必须! 否则 -mfloat-abi=hard 编译会失败)
 *
 * Keil / IAR / GCC 编译选项:
 *   -mcpu=cortex-m4 -mfloat-abi=hard -mfpu=fpv4-sp-d16
 *   -O2 -ffast-math -funroll-loops -DNDEBUG
 *
 * 链接 (GCC, linker script 见 stm32f407_flash.ld):
 *   把 .weights 段放在 Flash 末尾, .ccmram 段放 CCMRAM (放权重 + 中间 buffer)
 * ============================================================================
 */
...
/* ── 业务参数 ──────────────────────────────────────────────────────── */
#define SCADA_ROWS_PER_WINDOW  CH32_WINDOW        /* 16 行 = 1 个窗口 */
#define SCADA_FEATURES_PER_ROW CH32_IN_CHANNELS    /* 19 维特征 */
#define ANOMALY_THRESHOLD      CH32_BEST_THRESHOLD /* 0.49 */

/* ── 业务状态 ──────────────────────────────────────────────────────── */
static float   scada_window[CH32_IN_SIZE];     /* 滚动窗口:19*16 floats */
 ...
 *   2. 在空闲中断里 parse 19 个 float
 *   3. 调 scada_push_row() 推入滚动窗口
 *   4. 累计 16 行后调 ch32_forward()
 *
 * void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart) {
 *     if (huart == &huart3) {
 *         float row[CH32_IN_CHANNELS];
 *         parse_modbus_row(rx_buf, row);   // 你自己实现
 *         scada_push_row(row);
 *     }
 * }
 */
...
static void scada_push_row(const float* row) {
    ...
    if (scada_row_idx >= SCADA_ROWS_PER_WINDOW) {
        ...
        float p = ch32_forward(scada_window, CH32_IN_SIZE);
        ...
        if (p >= ANOMALY_THRESHOLD) {
            ...
            led_on();
            buzzer_on();
            printf("[W%lu] ⚠ ANOMALY  p=%.4f  latency=%.1f us  (anomaly=%lu/%lu)\n",
            ...
            if (g_window_count % 10 == 0) {
                printf("[W%lu]   normal    p=%.4f  latency=%.1f us  (anomaly=%lu/%lu)\n",
                       g_window_count, p, us, g_anomaly_count, g_window_count);
            }
        }
    }
...
int main(void) {
    ...
    printf("  Model: 19-dim SCADA, window=16, 3 blocks, d=[1,2,4]\n");
    ...
    /* 5. 内存自检 */
    ch32_mem_info_t m = ch32_get_mem_info();
    printf("[mem]  weights FP32 = %.2f KB\n", m.weights_fp32_bytes / 1024.0f);
...
/* ============================================================================
 * STM32 编译命令 (Keil MDK / IAR / GCC 三选一)
 * ============================================================================
 *
 * Keil MDK:
 *   - Project → Options for Target → C/C++ → Optimization: -O2
 *   - Target → Floating Point Hardware: Use Single Precision
 *   - 添加 main_stm32f407.c, ch32_inference.c, model_ch32_hybrid.h, ch32_inference.h
 *   - 添加 stm32f407_flash.ld  (如果用 GCC 的话)
 *
 * arm-none-eabi-gcc (Makefile 片段):
 *   CFLAGS  = -mcpu=cortex-m4 -mfloat-abi=hard -mfpu=fpv4-sp-d16 \
 *             -O2 -ffast-math -funroll-loops -DNDEBUG -std=c99
 *   LDFLAGS = -T stm32f407_flash.ld -mcpu=cortex-m4 -mfloat-abi=hard \
 *             -mfpu=fpv4-sp-d16 --specs=nosys.specs
 *   SRCS    = main_stm32f407.c ch32_inference.c \
 *             Drivers/STM32F4xx_HAL_Driver/Src/*.c startup_stm32f407xx.s
 *
 * OpenOCD 烧录:
 *   openocd -f interface/stlink.cfg -f target/stm32f4x.cfg \
 *           -c "program build/ch32_detect.elf verify reset exit"
 * ============================================================================
 */