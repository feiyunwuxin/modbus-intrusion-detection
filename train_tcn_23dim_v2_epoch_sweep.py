#!/usr/bin/env python3
"""TCN+SE 23-dim + B=64 + LR=4e-3 (LR sweep champion) × Epoch 扫描 + 5-seed

固定超参 (来自本工作 3 轮 sweep 累积):
  - 23-dim = 27-dim - {length(2), setpoint(3), crc_mean_w(20), cmd_count_w(22)}
  - BATCH = 64 (batch sweep 甜蜜点)
  - LR = 4e-3 (LR sweep 甜蜜点)
  - WD = 1e-5, Dropout = 0.3
  - ch=64, k=3, dilations=[1,2,4], SE r=8, W=16
  - patience = 5 (与所有 sweep 一致)

扫描 epoch ∈ {20, 30, 40, 50, 60}  × 5 seeds = 25 runs

历史依据: [[project-hparam-sweep-comprehensive]] 在 19-dim + B=128 + LR=2e-3 + ep=40
          得到 5-seed ensemble F1m=0.8735 (历史冠军)

用法 (单跑):
    python train_tcn_23dim_v2_epoch_sweep.py --epoch 40 --seed 42
"""
...
def run_one(epochs: int, seed: int):
...
def main():
    ...
    parser.add_argument("--epoch", type=int, required=True, choices=EPOCH_GRID)
    parser.add_argument("--seed", type=int, required=True, choices=SEEDS)
    parser.add_argument("--out", type=str, default=None)
    args = parser.parse_args()

    if args.out is None:
        args.out = os.path.join(BASE, f"train_tcn_23dim_v2_epoch_sweep_partial_ep{args.epoch}_s{args.seed}.json")

    print(f"[run] epoch={args.epoch}  seed={args.seed}", flush=True)
    t0 = time.time()
    r = run_one(args.epoch, args.seed)
