#!/usr/bin/env python3
"""
TCN+SE 19-dim SCADA — EXTREME long training: B=128, EPOCHS=200.

Goal: trace the full overfitting curve. Earlier 50ep showed val↑test↓,
this run should reveal the inflection point and plateau clearly.

Configuration:
  - BATCH = 128 (the sweet spot from sweep)
  - EPOCHS = 200 (5× the 40-epoch baseline)
  - PATIENCE = 30 (very patient — let it run all 200 unless val stalls hard)
  - LR scheduler = ReduceLROnPlateau (mode='max', factor=0.5, patience=2)
    → LR decays naturally as val F1m plateaus
  - min LR floor implicit via very small initial LR

Expected output: a long training history CSV with 200 epochs of data
showing exactly when the model starts overfitting.
"""
...
SEED         = 42
WINDOW       = 16
EPOCHS       = 200          # was 40 ★ EXTREME
BATCH        = 128          # sweet spot
LR           = 5e-4
WD           = 1e-5
...
print(f"[config] BATCH={BATCH}  EPOCHS={EPOCHS}  PATIENCE={PATIENCE}  → ~{EPOCHS * 3 // 60} min expected")
...
log(f"   train windows: {X_train_w.shape}, iters/epoch: {len(X_train_w) // BATCH}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    # Log every 5 epochs to keep output manageable
    if epoch % 5 == 0 or epoch <= 5:
        log(f"   epoch {epoch:3d}  loss={train_loss:.4f}  val Macro-F1={val_f1m:.4f}  val AUC={val_auc:.4f}  lr={cur_lr:.2e}")
...
torch.save({
    ...
    "total_updates": total_updates, "lr": LR,
...
meta = {
    ...
    "ablation_type": "v4+SE with BATCH=128, EPOCHS=200 (extreme)",
    "window": WINDOW,
...
log(f"   vs B=128 ep=40 (0.8655): ΔMacro-F1 = {test_thr['macro_f1'] - 0.8655:+.4f}  ΔPR-AUC = {test_thr['pr_auc'] - 0.9122:+.4f}")
log("=" * 60)