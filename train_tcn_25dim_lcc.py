#!/usr/bin/env python3
"""25 维 = 27 - {length (idx 2), cmd_count_w (idx 22)} retrain 5-seed 验证

5-seed LOO 单点预测:
  - length (idx 2):       DeltaF1m = +0.0051 (正向最大)
  - cmd_count_w (idx 22): DeltaF1m = +0.0006 (正向,几乎中性)

LOO 加总预测: +0.0051 + 0.0006 = +0.0057
期望 F1m >= 0.8205 + 0.0057 = 0.8262
"""
...
# 25 维 = 27 - {2 length, 22 cmd_count_w}
KEEP_25 = [i for i in range(27) if i not in (2, 22)]
DROP = {2: "length", 22: "cmd_count_w"}
...
print(f"=== TCN+SE 25-dim (-length, -cmd_count_w) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
...
print("\n=== 25-dim (-length, -cmd_count_w) 5-seed ===")
results = []
for seed in SEEDS:
...
print("\n" + "="*80)
print("=== 25-dim (-length, -cmd_count_w) 5-seed 汇总 ===")
print("="*80)
...
print(f"  LOO 加总预测:        DeltaF1m >= +0.0057 (= +0.0051 + 0.0006)")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0057 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0057 else 'NO FAIL'}")
...
out_json = os.path.join(BASE, "train_tcn_25dim_lcc_results.json")
