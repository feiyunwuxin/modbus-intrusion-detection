#!/usr/bin/env python3
"""
Ensemble v2 — Stack ALL_7: extends v1 with LSTM+Multi-Head Self-Attention.

Adds the new PR-AUC champion (LSTM+Attention, PR-AUC=0.9088) to the v1 lineup.
Tests whether the meta-learner can extract additional signal.

Compared to v1:
  - Stacking with 7 base models
  - Same validation-aligned, threshold-tuning-on-val protocol
  - Same 3 ensembling methods (uniform / weighted SLSQP / LR stacking)
"""
...
# All 7 base models. v1 had 6, v2 adds LSTM+Attention.
MODELS = [
...
for name, vf, tf, kind, win in MODELS:
    ...
    log(f"   {name:10s}  val: {v.shape}  test: {t.shape}")
...
for name in val_dfs:
    ...
    r = indiv_results[-1]
    log(f"   {name:10s}  thr={r['best_thr']:.2f}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}  Bin-F1={r['test_f1b']:.4f}  Acc={r['test_acc']:.4f}")
...
# ─────────────────────────────────────────────
# Subsets to try (v1's 7 + new ones with LSTM+Att)
# ─────────────────────────────────────────────
SUBSETS = [
    ("ALL_7",                       names),
    ("LGB+RF+TCN_v4+LSTM_Att",     ["LGB v2", "RF", "TCN v4", "LSTM+Att"]),
    ("LGB+TCN_v4+LSTM_Att",        ["LGB v2", "TCN v4", "LSTM+Att"]),
    ("LGB+RF+LSTM_Att",            ["LGB v2", "RF", "LSTM+Att"]),
    ("LGB+TCN_v4+v3c+LSTM_Att",    ["LGB v2", "TCN v4", "TCN v3c", "LSTM+Att"]),  # 2 best PR-AUC models
    ("LGB+TCN_v3c+LSTM_Att",       ["LGB v2", "TCN v3c", "LSTM+Att"]),             # 2 best PR-AUC only
    ("LGB+TCN_v4+v3a+v3c+LSTM_Att",["LGB v2", "TCN v4", "TCN v3a", "TCN v3c", "LSTM+Att"]),
]
...
for label, members in SUBSETS:
    ...
    r = uniform_results[-1]
    log(f"   {label:32s}  thr={r['best_thr']:.2f}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}")
...
for label, members in SUBSETS:
    ...
    log(f"   {label:32s}  weights={w.round(2).tolist()}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}")
...
for label, members in SUBSETS:
    ...
    r = stack_results[-1]
    log(f"   {label:32s}  C={best_C}  F1m={r['test_f1m']:.4f}  PR-AUC={r['test_pra']:.4f}  coefs={list(r['coefs'].values())}")

# ─────────────────────────────────────────────
log("STEP 7: save artifacts ...")
OUT  = os.path.join(BASE, "evaluation_ensemble_v2.txt")
META = os.path.join(BASE, "processed_meta_ensemble_v2.json")

lines = [
    "=" * 80, "ENSEMBLE REPORT — v2: ALL_7 (LGB + RF + TCN_v4 + v3a + v3b + v3c + LSTM+Att)",
    "=" * 80,
    "Base models:", ""
]
