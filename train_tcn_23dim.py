#!/usr/bin/env python3
"""23 维精简版 retrain 验证 - LOO 加总预测 DeltaF1m >= +0.02

去除特征:
  - 3  setpoint               (DeltaF1m=+0.0074, noise)
  - 6  deadband               (DeltaF1m=+0.0090, noise)
  - 16 time_since_last_same_addr_func (DeltaF1m=+0.0004, noise)
  - 20 crc_mean_w             (DeltaF1m=+0.0040, noise)

LOO 加总预测: DeltaF1m = +0.0074+0.0090+0.0004+0.0040 = +0.0208
期望 F1m >= 0.8268 + 0.020 = 0.847
...
print(f"=== TCN+SE 23-dim Retrain 验证 ===")
print(f"去除特征: {DROP}")
print(f"保留特征 ({len(KEEP_23)}): {[FEATURE_NAMES_27[i] for i in KEEP_23]}")
...
# 对比 baseline (27 dim, seed=42)
print("\n=== Baseline 27-dim (复现) ===")
baseline_27 = list(range(27))
r27 = run_one(baseline_27, seed=42)
print(f"  F1m={r27['test_macro_f1']:.4f}  PR-AUC={r27['test_pr_auc']:.4f}  Epoch={r27['best_epoch']}  Params={r27['n_params']:,}")
...
for seed in seeds:
    print(f"\n=== 23-dim seed={seed} ===")
    r = run_one(KEEP_23, seed=seed)
    print(f"  F1m={r['test_macro_f1']:.4f}  PR-AUC={r['test_pr_auc']:.4f}  Epoch={r['best_epoch']}  Params={r['n_params']:,}")
    results.append(r)

# 汇总
print("\n" + "="*80)
print("=== 23-dim 5-seed 汇总 ===")
print("="*80)
...
print(f"\n{'mean':>5} {mean_f1m:>8.4f} (std={std_f1m:.4f}) {' '*24} {mean_pr:>8.4f} (std={std_pr:.4f})")
print(f"\nDelta vs 27-dim baseline (seed=42, F1m={r27['test_macro_f1']:.4f}):")
print(f"  23-dim mean:        F1m = {mean_f1m:.4f}  DeltaF1m = {mean_f1m - r27['test_macro_f1']:+.4f}")
print(f"  23-dim seed=42:     F1m = {results[0]['test_macro_f1']:.4f}  DeltaF1m = {results[0]['test_macro_f1'] - r27['test_macro_f1']:+.4f}")
print(f"  LOO 加总预测:        DeltaF1m >= +0.0208")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - r27['test_macro_f1'] >= 0.020 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed)? {'YES OK' if results[0]['test_macro_f1'] - r27['test_macro_f1'] >= 0.020 else 'NO FAIL'}")

# 保存
out_json = os.path.join(BASE, "train_tcn_23dim_results.json")
with open(out_json, "w") as f:
    json.dump({"baseline_27dim": r27, "results_23dim": results,
               "mean_f1m": mean_f1m, "std_f1m": std_f1m,
               "mean_pr_auc": mean_pr, "std_pr_auc": std_pr,
               "drop_features": DROP, "keep_idx": KEEP_23}, f, indent=2)
print(f"\nSaved: {out_json}")