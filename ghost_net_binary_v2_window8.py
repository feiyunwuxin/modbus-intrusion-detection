#!/usr/bin/env python3
"""
1D GhostNet binary classifier on the 17-feature v2 dataset, sliding window=8.

GhostNet key idea: instead of computing all output channels with expensive convolutions,
generate them in two steps:
1. A few "intrinsic" features via a standard 1×1 conv (1/s of the output channels)
2. Many "ghost" features via cheap depthwise convolutions on the intrinsic ones
The concatenation gives the full output at a fraction of the FLOPs and params.

For comparison:
- Standard conv: out_ch × in_ch × k   (e.g., 128 × 64 × 3 = 24,576 params)
- MobileNet (depthwise-separable): in_ch × k + in_ch × out_ch = 192 + 8,192 = 8,384
- GhostNet (with ratio=2): (in_ch × out_ch/2 × 1) + (out_ch/2 × k × depthwise) = 4,096 + 192 = 4,288
"""
...
MODEL_OUT = os.path.join(BASE, "model_ghost_net_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_ghost_net_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_ghost_net_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_ghost_net_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_ghost_net_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_ghost_net_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_ghost_net_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_ghost_net_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_ghost_net_binary_v2_window8.png")
...
CLIP_VAL    = 10.0
# GhostNet config
INIT_CHANNELS  = 64           # initial conv output
GHOST_RATIO    = 2            # intrinsic : ghost = 1 : 1 (so out_ch / 2 intrinsic)
# Ghost bottleneck channels: (out_ch, stride, kernel_size)
BOTTLENECKS = [
    (128, 1, 3),    # expansion
    (128, 1, 3),    # same
    (256, 1, 3),    # expand more
...
log(f"   final GhostNet input shape: {X_train_cnn.shape}  → Conv1D(F={N_FEATURES}, W={WINDOW})")
...
class GhostModule1D(nn.Module):
    """Generate out_ch features as: intrinsic (out_ch/ratio via 1x1 conv) + ghost (cheap depthwise)."""
    def __init__(self, in_ch, out_ch, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True):
        super().__init__()
        self.out_ch = out_ch
        init_ch = out_ch // ratio
        new_ch = out_ch - init_ch   # ghost features count
        # Primary (intrinsic) features via 1x1 conv
        self.primary_conv = nn.Sequential(
            nn.Conv1d(in_ch, init_ch, kernel_size, stride=stride,
                      padding=kernel_size//2, bias=False),
            nn.BatchNorm1d(init_ch),
            nn.ReLU(inplace=True) if relu else nn.Identity(),
        )
        # Ghost features via cheap depthwise conv on intrinsic features
        self.cheap_conv = nn.Sequential(
            nn.Conv1d(init_ch, new_ch, dw_size, padding=dw_size//2, groups=init_ch, bias=False),
...
class GhostBottleneck1D(nn.Module):
    """Ghost bottleneck: expansion ghost module + depthwise + compression ghost module + residual."""
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, ratio=2, dropout=0.0):
        super().__init__()
        # Expansion: in_ch → 2 * in_ch (or in_ch → out_ch if in_ch < out_ch)
        hidden_ch = max(in_ch, out_ch)  # expand by 2x (or to out_ch)
        self.conv = nn.Sequential(
            GhostModule1D(in_ch, hidden_ch, kernel_size=1, ratio=ratio),
            # Depthwise
            ...
            # Compression
            GhostModule1D(hidden_ch, out_ch, kernel_size=1, ratio=ratio, relu=False),
            nn.Dropout(dropout) if dropout > 0 else nn.Identity(),
        )
        # Residual: identity if shape matches, else 1x1 conv
...
class GhostNet1D(nn.Module):
    def __init__(self, in_ch, init_channels=INIT_CHANNELS, bottlenecks=BOTTLENECKS,
                 ratio=GHOST_RATIO, dropout=DROPOUT):
        super().__init__()
        # Stem
        self.stem = nn.Sequential(
            nn.Conv1d(in_ch, init_channels, 3, padding=1, bias=False),
        ...
        layers = []
        in_c = init_channels
        for out_c, stride, k in bottlenecks:
            layers.append(GhostBottleneck1D(in_c, out_c, kernel_size=k, stride=stride, ratio=ratio, dropout=dropout))
            in_c = out_c
        self.bottlenecks = nn.Sequential(*layers)
        # Classifier head
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.fc1 = nn.Linear(in_c, 32)
...
model = GhostNet1D(in_ch=N_FEATURES)
n_params = sum(p.numel() for p in model.parameters())
log(f"   model params: {n_params:,}")
log(f"   architecture: Stem({N_FEATURES}→{INIT_CHANNELS}, k=3) → "
    f"{len(BOTTLENECKS)} Ghost bottlenecks {BOTTLENECKS} → GAP → FC(32→1)")
log(f"   ghost_ratio = {GHOST_RATIO} (intrinsic:ghost = 1:{GHOST_RATIO-1})")
...
meta = {
    "model": f"GhostNet1D v2 window={WINDOW}",
    "window": WINDOW,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — GhostNet1D Binary (v2, sliding window=8)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: Stem({N_FEATURES}→{INIT_CHANNELS}, k=3) → {len(BOTTLENECKS)} Ghost bottlenecks (ghost_ratio={GHOST_RATIO}) → GAP → FC(32→1)")
lines.append(f"Bottleneck channels: {[c for c, _, _ in BOTTLENECKS]}")
...
log(f"   GhostNet      : stem={INIT_CHANNELS}, {len(BOTTLENECKS)} bottlenecks, ghost_ratio={GHOST_RATIO}")
