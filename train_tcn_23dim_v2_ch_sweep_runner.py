#!/usr/bin/env python3
"""TCN+SE 23-dim + B=64 + LR=4e-3 + ep=20 × Channels 扫描 — 5-way parallel runner

25 runs (5 channels × 5 seeds). 5 workers × 5 runs each (1 seed × 5 channels).
"""
...
def main():
    ...
    for wid, seed in enumerate(SEEDS):
        wid_path = os.path.join(BASE, f"_worker_ch23v2_w{wid}.py")
        with open(wid_path, "w", encoding="utf-8") as f:
            f.write(f'''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Worker {wid} seed={seed}, 5 channels"""
import os, sys, time, json
sys.path.insert(0, r"{BASE}")
from train_tcn_23dim_v2_ch_sweep import run_one

MY_SEED = {seed}
MY_CHS = {CHANNELS_GRID}
WORKER_ID = {wid}
BASE = r"{BASE}"

t0 = time.time()
for ch in MY_CHS:
    out = os.path.join(BASE, f"train_tcn_23dim_v2_ch_sweep_partial_ch{{ch}}_s{{MY_SEED}}.json")
    ...
    t1 = time.time()
    try:
        r = run_one(ch, MY_SEED)
        r["ch_param"] = ch
        r["seed"] = MY_SEED
        with open(out, "w", encoding="utf-8") as f:
            json.dump(r, f, indent=2)
        print(f"[w{{WORKER_ID}}] [done] ch={{ch}} s={{MY_SEED}} "
...
''')
        log_path = os.path.join(BASE, f"_worker_ch23v2_w{wid}.log")
