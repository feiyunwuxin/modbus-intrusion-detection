#!/usr/bin/env python3
"""
TCN v4 +SE on PREPROCESSED v2 (SCADA protocol features) data.

Same architecture as the v4 baseline (TCN + SE), but input is 27-dim
(19 row-level + 8 window-aggregate features from preprocess_v2_scada.py).
"""
...
# TCN v4 hyperparameters (SAME as v4 baseline)
SEED = 42
WINDOW = 16
EPOCHS = 35
BATCH = 512
...
log(f"   TCN v4 +SE on 27-dim input, params: {n_params:,}")
...
torch.save({"state_dict": best_state, "n_features": N_FEATURES, "window": WINDOW,
            ...
            "best_val_f1m": best_f1m, "preprocessing": "v2_scada"}, MODEL_OUT)
...
meta = {
    "model": f"TCN v4 +SE on v2 data",
    "tag": TAG, "preprocessing": "v2_scada", "window": WINDOW,
...
lines = [
    "=" * 72, f"EVALUATION REPORT — TCN v4 +SE on v2 (SCADA-aware features)",
    "=" * 72,
...
log(f"   vs TCN v4 baseline (0.8340): ΔMacro-F1 = {test_thr['macro_f1'] - 0.8340:+.4f}")
log(f"   vs TCN v4 baseline (0.9025): ΔPR-AUC   = {test_thr['pr_auc'] - 0.9025:+.4f}")
log("=" * 60)
