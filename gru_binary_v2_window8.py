#!/usr/bin/env python3
"""
Bidirectional GRU binary classifier on the 17-feature v2 dataset, sliding window=8.
- GRU has 2 gates (update + reset) vs LSTM's 3 gates + cell state → fewer params
- Direct comparison with BiLSTM w=8 (same hyperparams, swap nn.LSTM → nn.GRU)
"""
...
MODEL_OUT = os.path.join(BASE, "model_gru_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_gru_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_gru_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_gru_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_gru_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_gru_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_gru_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_gru_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_gru_binary_v2_window8.png")

# Hyperparameters (same as BiLSTM w=8 for fair comparison)
SEED        = 42
...
log(f"   final GRU input shape: {X_train_cnn.shape}  → BiGRU(T={WINDOW}, F={N_FEATURES})")
...
meta = {
    "model": f"BiGRU v2 window={WINDOW}",
    "window": WINDOW,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — BiGRU Binary (v2, sliding window=8)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: {NUM_LAYERS}-layer {'Bi' if BIDIRECTIONAL else ''}GRU, hidden={HIDDEN}, dropout={DROPOUT}")
