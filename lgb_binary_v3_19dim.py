#!/usr/bin/env python3
"""
LightGBM binary classifier on the 19-feature v2_scada dataset, row-level.
...
# 19-dim v2_scada row-level data (already preprocessed)
...
MODEL_OUT  = os.path.join(BASE, "model_lgb_binary_v3_19dim.joblib")
EVAL_OUT   = os.path.join(BASE, "evaluation_lgb_binary_v3_19dim.txt")
IMP_OUT    = os.path.join(BASE, "feature_importance_lgb_binary_v3_19dim.csv")
META_OUT   = os.path.join(BASE, "processed_meta_lgb_binary_v3_19dim.json")
THR_OUT    = os.path.join(BASE, "best_threshold_lgb_binary_v3_19dim.json")
PRED_VAL   = os.path.join(BASE, "predictions_val_lgb_binary_v3_19dim.csv")
PRED_TEST  = os.path.join(BASE, "predictions_test_lgb_binary_v3_19dim.csv")
CM_PNG     = os.path.join(BASE, "confusion_matrix_lgb_binary_v3_19dim.png")
PRROC_PNG  = os.path.join(BASE, "pr_roc_lgb_binary_v3_19dim.png")
...
# Wrap in DataFrame for LGB categorical handling
X_train = pd.DataFrame(X_train, columns=FEATURE_NAMES)
X_val   = pd.DataFrame(X_val,   columns=FEATURE_NAMES)
X_test  = pd.DataFrame(X_test,  columns=FEATURE_NAMES)
X_train["function"] = X_train["function"].astype("category")
X_val["function"]   = X_val["function"].astype("category")
X_test["function"]  = X_test["function"].astype("category")

# ─────────────────────────────────────────────
log("STEP 2: LightGBM training ...")
...
meta = {
    "model": "LightGBM v3 19-dim", "n_features": 19, "feature_names": FEATURE_NAMES,
    "best_iteration": int(clf.best_iteration_), "best_threshold": best_thr,
...
lines = [
    "=" * 72, "EVALUATION REPORT — LightGBM binary v3 (19-dim SCADA row-level)",
    "=" * 72,
    f"Train: {len(X_train):,}  |  Val: {len(X_val):,}  |  Test: {len(X_test):,}",
    f"Features: 19  |  Best LGB iteration: {clf.best_iteration_}",
...
try:
    ...
    ax.set_title(f"LGB v3 19-dim  thr={best_thr:.2f}  Macro-F1={test_thr['macro_f1']:.4f}")
