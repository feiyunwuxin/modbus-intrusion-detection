#!/usr/bin/env python3
"""
LSTM binary classifier on the 17-feature v2 dataset, **sliding window of 8 rows**
(2 Modbus cycles = 8 timesteps × 17 features). Bidirectional LSTM to maximize
information extraction across the longer sequence.

Pipeline:
- Load npy arrays from pipeline_binary.py
- Group every 8 consecutive rows into 1 window → shape (n_windows, 8, 17)
- One-hot encode function for each of the 8 timesteps
- Window label = 1 if any of the 8 rows has y=1
- BiLSTM over the 8 timesteps, concatenate final forward + backward hidden states
- Val-based threshold tuning (no test leak)
- Compare against LGB v2, single-row CNN, window=4 CNN, window=4 LSTM
"""
...
MODEL_OUT = os.path.join(BASE, "model_lstm_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_lstm_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_lstm_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_lstm_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_lstm_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_lstm_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_lstm_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_lstm_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_lstm_binary_v2_window8.png")
...
# ─────────────────────────────────────────────
# Step 2 — Group every WINDOW rows into 1 cycle
# ─────────────────────────────────────────────
log(f"STEP 2: group every {WINDOW} rows into 1 window ...")
def make_windows(X, y, win):
    n = (len(X) // win) * win
    Xw = X[:n].reshape(n // win, win, -1)
    yw = (y[:n].reshape(n // win, win).max(axis=1)).astype(np.int64)  # 1 if any row in cycle is attack
    return Xw, yw
...
log(f"   train cycles: {X_train_w.shape} (dropped {len(X_train) - len(X_train_w)*WINDOW} tail rows)")
log(f"   val   cycles: {X_val_w.shape}   (dropped {len(X_val)   - len(X_val_w)*WINDOW} tail rows)")
log(f"   test  cycles: {X_test_w.shape}  (dropped {len(X_test)  - len(X_test_w)*WINDOW} tail rows)")
log(f"   cycle-label dist: train 0={int((y_train_w==0).sum()):,} 1={int((y_train_w==1).sum()):,} ({y_train_w.mean()*100:.2f}% attack)")
...
# ─────────────────────────────────────────────
# Step 3 — One-hot encode `function` (col 1) at each timestep
# ─────────────────────────────────────────────
...
# Clip to prevent LSTM input overflow
...
log(f"   final LSTM input shape: {X_train_cnn.shape}  → LSTM(T={WINDOW}, F={N_FEATURES})")
...
class LSTMClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            # Concat last forward + last backward hidden
...
# pos_weight for imbalance (cycle-level)
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — LSTM Binary (v2, sliding window=8, BiLSTM)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows per cycle  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: {NUM_LAYERS}-layer {'Bi' if BIDIRECTIONAL else ''}LSTM, hidden={HIDDEN}, dropout={DROPOUT}")
lines.append(f"Train cycles: {len(X_train_w):,}  |  Val cycles: {len(X_val_w):,}  |  Test cycles: {len(X_test_w):,}")
...
log("=" * 60)
log("DONE.")
log(f"   window        : {WINDOW} rows per cycle")
log(f"   features/step : {N_FEATURES} (16 raw + {n_cats} one-hot)")
log(f"   train/val/test cycles: {len(X_train_w):,} / {len(X_val_w):,} / {len(X_test_w):,}")
