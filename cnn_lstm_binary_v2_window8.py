#!/usr/bin/env python3
"""
CNN-LSTM hybrid binary classifier on the 17-feature v2 dataset, sliding window=8.
- Conv1D front-end extracts local 2-3 row patterns from each step
- BiLSTM back-end models temporal dependencies across the 8 timesteps
- This combines the strengths of:
  * Window=4 CNN: efficient local pattern detection
  * BiLSTM w=8: bidirectional temporal modeling
"""
...
MODEL_OUT = os.path.join(BASE, "model_cnn_lstm_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_cnn_lstm_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_cnn_lstm_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_cnn_lstm_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_cnn_lstm_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_cnn_lstm_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_cnn_lstm_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_cnn_lstm_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_cnn_lstm_binary_v2_window8.png")
...
CLIP_VAL    = 10.0
# CNN front-end
CONV_CHANNELS = [128, 128]            # 2 conv layers
CONV_KERNEL   = 3
# LSTM back-end
HIDDEN        = 64
NUM_LAYERS    = 1
...
log(f"   final CNN-LSTM input shape: {X_train_cnn.shape}  → Conv1D(F={N_FEATURES}, W={WINDOW})")
...
class CNNLSTMClassifier(nn.Module):
    """
    Conv1D front-end (local patterns) + BiLSTM back-end (temporal modeling).
    Input: (B, F, T)  →  Conv1D over T  →  permute  →  BiLSTM over T  →  final hidden  →  FC
    """
    def __init__(self, n_features, conv_channels, conv_kernel, window,
                 hidden=HIDDEN, num_layers=NUM_LAYERS, dropout=DROPOUT,
                 bidirectional=BIDIRECTIONAL):
        super().__init__()
        self.bidirectional = bidirectional

        # CNN front-end: stack of Conv1D + BN + ReLU + Dropout
        layers = []
        in_ch = n_features
        for out_ch in conv_channels:
            layers += [
                nn.Conv1d(in_ch, out_ch, kernel_size=conv_kernel, padding=conv_kernel // 2),
        ...
        cnn_out_ch = conv_channels[-1]

        # LSTM back-end on the CNN features
        self.lstm = nn.LSTM(
...
log(f"   architecture: Conv1D({N_FEATURES}→{'→'.join(map(str, CONV_CHANNELS))}, k={CONV_KERNEL}) "
    f"→ BiLSTM(C={CONV_CHANNELS[-1]}, H={HIDDEN}) → FC({HIDDEN*2}→32→1)")
...
meta = {
    "model": f"CNN-LSTM v2 window={WINDOW}",
    "window": WINDOW,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — CNN-LSTM Binary (v2, sliding window=8, BiLSTM)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: Conv1D({N_FEATURES}→{'→'.join(map(str, CONV_CHANNELS))}, k={CONV_KERNEL}, pad=same) "
             f"→ BiLSTM(in={CONV_CHANNELS[-1]}, hidden={HIDDEN}) → FC({HIDDEN*2}→32→1)")
...
log(f"   conv channels : {CONV_CHANNELS}, kernel={CONV_KERNEL}")
log(f"   BiLSTM        : hidden={HIDDEN}, num_layers={NUM_LAYERS}, bidirectional={BIDIRECTIONAL}")
