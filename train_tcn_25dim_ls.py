#!/usr/bin/env python3
"""25 维 = 27 - {length (idx 2), setpoint (idx 3)} retrain 5-seed 验证

5-seed LOO 单点预测:
  - length (idx 2):   DeltaF1m = +0.0051 (正向最大)
  - setpoint (idx 3): DeltaF1m = +0.0003 (几乎中性)

LOO 加总预测: +0.0051 + 0.0003 = +0.0054
期望 F1m >= 0.8205 + 0.0054 = 0.8259
"""
...
# 25 维 = 27 - {2 length, 3 setpoint}
KEEP_25 = [i for i in range(27) if i not in (2, 3)]
DROP = {2: "length", 3: "setpoint"}
...
print(f"=== TCN+SE 25-dim (-length, -setpoint) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
...
print("\n" + "="*80)
print("=== 25-dim (-length, -setpoint) 5-seed 汇总 ===")
print("="*80)
...
print(f"  LOO 加总预测:        DeltaF1m >= +0.0054 (= +0.0051 + 0.0003)")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0054 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0054 else 'NO FAIL'}")
...
out_json = os.path.join(BASE, "train_tcn_25dim_ls_results.json")
