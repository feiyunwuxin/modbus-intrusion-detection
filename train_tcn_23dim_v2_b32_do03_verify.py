#!/usr/bin/env python3
"""TCN+SE 23-dim + B=32 + LR=4e-3 + ep=20 + ch=64 + do=0.3 × 5-seed — 2D 验证

测试: B=64→B=32, 但保持 do=0.3 (vs 之前 do=0.1 验证)。
      B=32 (376 iters/ep) + 强正则化 = 探索不同角落
对照: B=64+do=0.3 baseline (F1m=0.8705) vs B=32+do=0.1 (F1m=0.8727)
"""
...
EPOCHS = 20
CHANNELS = 64
DROPOUT = 0.3             # 改为 0.3 (vs 之前 0.1 验证)
WD = 1e-5
PATIENCE = 5
GRAD_CLIP = 0.5
CLIP_VAL = 10.0
...
def main():
    ...
    if args.out is None:
        args.out = os.path.join(BASE, f"train_tcn_23dim_v2_b32_do03_verify_partial_s{args.seed}.json")

    print(f"[run] B=32 do=0.3 seed={args.seed}", flush=True)
    t0 = time.time()
    r = run_one(args.seed)
