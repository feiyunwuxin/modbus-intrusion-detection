#!/usr/bin/env python3
"""
Unidirectional VANILLA (basic) RNN binary classifier on the 17-feature v2 dataset,
sliding window=8.

Cell-type ablation: BasicRNN (1 gate) vs GRU (2 gates) vs LSTM (3 gates).
Same hyperparams, only the cell type changes. Quantifies the value of gating on this task.

Cell comparison:
  Vanilla RNN: h_t = tanh(W_ih · x_t + W_hh · h_{t-1} + b)
    → 1 weight matrix, 1 activation, NO gates
    → Fewest parameters; classic vanishing-gradient problem on long sequences
  GRU:        h_t = (1-z)·h_{t-1} + z·h̃_t   (z = update gate, r = reset gate)
    → 2 gates, no separate cell state
  LSTM:       i,f,o gates + cell state c_t
    → 3 gates + cell state, most expressive

vs GRU Uni w=8 (same architecture, only GRU → RNN):
  Expected params: ~10K (vs GRU 23K)
  Expected performance: slightly worse (no gates = harder to learn long dependencies)
"""
...
# Output naming — distinguish from GRU/LSTM artifacts
MODEL_OUT = os.path.join(BASE, "model_rnn_basic_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_rnn_basic_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_rnn_basic_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_rnn_basic_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_rnn_basic_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_rnn_basic_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_rnn_basic_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_rnn_basic_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_rnn_basic_binary_v2_window8.png")

# Hyperparameters — IDENTICAL to GRU Uni w=8 (only the cell type changes)
SEED        = 42
...
NUM_LAYERS  = 1
BIDIRECTIONAL = False
NONLINEARITY = "tanh"                 # vanilla RNN default; 'relu' is the other option
...
log(f"   final RNN input shape: {X_train_cnn.shape}  → vanilla RNN(T={WINDOW}, F={N_FEATURES}, unidirectional)")
...
# ─────────────────────────────────────────────
# Step 5 — Vanilla RNN (basic, no gating)
# ─────────────────────────────────────────────
...
model = RNNClassifier(N_FEATURES)
n_params = sum(p.numel() for p in model.parameters())
log(f"   model params: {n_params:,}")
log(f"   architecture: 1-layer UNIDIRECTIONAL vanilla RNN, hidden={HIDDEN}, nonlinearity={NONLINEARITY}, dropout={DROPOUT}")
log(f"                  → h[-1] (last timestep, last layer) → FC({HIDDEN}→32) → FC(32→1)")

# Reference: GRU Uni has out_dim = 64, params 23,233
# Vanilla RNN has out_dim = 64, expected params ≈ 10K (1 weight matrix + tanh)
...
meta = {
    "model": f"Vanilla RNN v2 window={WINDOW}",
    "window": WINDOW,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — Vanilla RNN Binary (v2, sliding window=8)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: 1-layer UNIDIRECTIONAL vanilla RNN, hidden={HIDDEN}, nonlinearity={NONLINEARITY}, dropout={DROPOUT}")
lines.append(f"vs GRU Uni w=8: same hyperparams, only cell type changes (1 gate → 2 gates)")
...
log(f"   architecture  : 1-layer unidirectional vanilla RNN (tanh), hidden={HIDDEN}")
log(f"   train/val/test windows: {len(X_train_w):,} / {len(X_val_w):,} / {len(X_test_w):,}")
log(f"   params        : {n_params:,}  (GRU Uni: 23,233)")
