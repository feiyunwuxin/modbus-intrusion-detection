#!/usr/bin/env python3
"""22 维 = 27 - {length (2), setpoint (3), press_mean_w (19), crc_mean_w (20), cmd_count_w (22)} retrain 5-seed 验证

5-seed LOO 单点预测:
  - length (2):          DeltaF1m = +0.0051
  - setpoint (3):        DeltaF1m = +0.0003
  - press_mean_w (19):   DeltaF1m = +0.0035
  - crc_mean_w (20):     DeltaF1m = +0.0010
  - cmd_count_w (22):    DeltaF1m = +0.0006

LOO 加总预测: +0.0051 + 0.0003 + 0.0035 + 0.0010 + 0.0006 = +0.0105
期望 F1m >= 0.8205 + 0.0105 = 0.8310
"""
...
# 22 维 = 27 - {2, 3, 19, 20, 22}
KEEP_22 = [i for i in range(27) if i not in (2, 3, 19, 20, 22)]
DROP = {2: "length", 3: "setpoint", 19: "press_mean_w", 20: "crc_mean_w", 22: "cmd_count_w"}
...
print(f"=== TCN+SE 22-dim (-length, -setpoint, -press_mean_w, -crc_mean_w, -cmd_count_w) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
print(f"保留特征 ({len(KEEP_22)} dim): {[FEATURE_NAMES_27[i] for i in KEEP_22]}")
...
print(f"  22-dim mean:        F1m = {mean_f1m:.4f}  DeltaF1m = {mean_f1m - baseline_f1m_5seed:+.4f}")
print(f"  22-dim seed=42:     F1m = {results[0]['test_macro_f1']:.4f}  DeltaF1m = {results[0]['test_macro_f1'] - baseline_f1m_5seed:+.4f}")
print(f"  LOO 加总预测:        DeltaF1m >= +0.0105 (= +0.0051 + 0.0003 + 0.0035 + 0.0010 + 0.0006)")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0105 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0105 else 'NO FAIL'}")
...
print(f"  22-dim mean:        DeltaF1m = {mean_f1m - single_champion_f1m:+.4f}")
...
out_json = os.path.join(BASE, "train_tcn_22dim_results.json")
with open(out_json, "w") as f:
    json.dump({"results_22dim": results,
               "mean_f1m": mean_f1m, "std_f1m": std_f1m,
               "mean_pr_auc": mean_pr, "std_pr_auc": std_pr,
               "drop_features": DROP, "keep_idx": KEEP_22}, f, indent=2)
print(f"\nSaved: {out_json}")