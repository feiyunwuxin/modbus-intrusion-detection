#!/usr/bin/env python3
"""
TCN v6 = TCN v4+SE + extra SE-pre block (between the two dilated convs).

Built on top of v3_v4se_window16 (19-dim SCADA, Macro-F1=0.8444 champion).

Single change vs v4+SE: add an SE block between conv1 and conv2 inside each residual block.
This gives a "screen → refine → screen" two-stage attention pattern:
  Conv1 → BN → ReLU → Drop → SE_pre  →  Conv2 → BN → ReLU → Drop → SE_post → Add ⊕ → ReLU
                          ────────                       ────────
                          NEW (v6)                      kept from v4+SE

Hypothesis:
  - SE_post (existing) weights "which OUTPUT channels of the block matter" (high-level)
  - SE_pre  (new)      weights "which INPUT channels to refine further" (mid-level)
  - Two SE = two-tier feature selection, analogous to a 2-stage attention cascade

Expected:
  - +0.001 ~ +0.003 Macro-F1 vs v4+SE 19-dim
  - +3,072 params (1,024 × 3 blocks)
  - +10 ~ +15% training time (extra SE forward passes)
"""
...
# ─────────────────────────────────────────────
# Step 4 — TCN v6 = Dual SE (inter-SE + post-SE)
# ─────────────────────────────────────────────
class SEBlock(nn.Module):
    """Squeeze-and-Excitation channel attention. Same as v4+SE."""
...
class TCNBlockDualSE(nn.Module):
    """TCN residual block with TWO SE blocks: SE-pre (between convs) + SE-post (after convs).

    Forward flow:
      x → Conv1 → BN → ReLU → Drop → SE_pre → Conv2 → BN → ReLU → Drop → SE_post → + residual → ReLU
                                  ──────                       ───────
                                  NEW                          kept from v4+SE
    """
    def __init__(self, in_ch, out_ch, kernel_size, dilation, dropout, se_reduction=SE_REDUCTION):
        ...
        self.se_pre  = SEBlock(out_ch, reduction=se_reduction)        # ← NEW (v6)
        self.se_post = SEBlock(out_ch, reduction=se_reduction)        # ← kept from v4+SE
    ...
    def forward(self, x):
        ...
        x = self.drop(x)
        x = self.se_pre(x)                                       # ← NEW (v6): between convs
        x = F.relu(self.bn2(self.conv2(x)))
        x = self.drop(x)
        x = self.se_post(x)                                      # ← kept (v4+SE): after convs
        return F.relu(x + residual)


class TCNClassifierDualSE(nn.Module):
    def __init__(self, in_ch, n_blocks=N_BLOCKS, channels=CHANNELS,
                 kernel_size=KERNEL_SIZE, dilations=DILATIONS, dropout=DROPOUT):
        super().__init__()
        layers = []
        layers.append(TCNBlockDualSE(in_ch, channels, kernel_size, dilations[0], dropout))
        for d in dilations[1:n_blocks]:
            layers.append(TCNBlockDualSE(channels, channels, kernel_size, d, dropout))
...
log(f"   DUAL SE: SE-pre (between convs) + SE-post (after convs)")
log(f"   Effective receptive field: {rf}  (window={WINDOW}, RF/W = {rf/WINDOW:.1f}x)")

model = TCNClassifierDualSE(in_ch=N_FEATURES)
n_params = sum(p.numel() for p in model.parameters())
log(f"   model params: {n_params:,}  (v4+SE was 72,921 → expect +3,072 = 75,993)")

# Count SE blocks
n_se_blocks = sum(1 for m in model.modules() if isinstance(m, SEBlock))
log(f"   SE blocks total: {n_se_blocks}  (was 3 in v4+SE)")
...
torch.save({
    ...
    "se_reduction": SE_REDUCTION, "n_se_blocks": n_se_blocks,
    "has_dual_se": True,
...
# Reference: TCN v3_v4se 19-dim scores (from processed_meta)
REF_F1M      = 0.8444
REF_PRAUC    = 0.8892
REF_BINARYF1 = 0.8347
REF_PARAMS   = 72921
...
meta = {
    ...
    "ablation_type": "v4+SE + extra SE-pre block between two convs (dual SE per block)",
    "window": WINDOW,
...
lines = [
    ...
    "=" * 72,
    f"Architecture: v3_v4se (19-dim) + DUAL SE per block (SE-pre between convs + SE-post after convs)",
    ...
    f"SE blocks: {n_se_blocks} total (was 3 in v4+SE; +1 per block = +3,072 params)",
