#!/usr/bin/env python3
"""
TCN v4 +SE on 19-dim SCADA row-level features (no window aggregates, no one-hot).

3-way fair comparison companion to:
  - v4_se_window16     → 17-dim (16 raw + time_diff) + one-hot function  = 44 Conv1D dims
  - v3_v4se_window16   → 19-dim SCADA row-level (this script)            = 19 Conv1D dims
  - v2_v4se_window16   → 19-dim SCADA row-level + 8 window aggregates     = 27 Conv1D dims

Source data: X_*_binary_v2_scada.npy (already saved from preprocess_v2_scada.py)
The 19 columns are:
  [0] address, [1] function (int, no one-hot),
  [2] length, [3..12] setpoint..solenoid, [13] pressure measurement, [14] crc rate,
  [15] time_diff, [16] time_since_last_same_addr_func,
  [17] is_unusual_fc, [18] is_response (alias of command response)

Window=16 grouping. No window-level aggregates.
Same TCN v4+SE architecture & hyperparameters as v4 baseline for fair comparison:
  N_BLOCKS=3, CHANNELS=64, KERNEL_SIZE=3, DILATIONS=[1,2,4], SE_REDUCTION=8.
"""
...
TAG = "v3_v4se_window16"
BASE = r"C:\work\Claude\Issue"

# v2 19-dim row-level data (no window aggregates, no one-hot)
...
# TCN v4 hyperparameters — IDENTICAL to v4 baseline + v2 (27-dim) for fair 3-way compare
SEED = 42
WINDOW = 16
EPOCHS = 35
BATCH = 512
...
# ─────────────────────────────────────────────
log("STEP 1: load 19-dim row-level npy ...")
...
log(f"   features per step: {N_FEATURES}  (v1=17→44 with one-hot, v2=27, v3=19 here)")
...
log(f"   TCN v4 +SE on 19-dim input, params: {n_params:,}")
...
torch.save({"state_dict": best_state, "n_features": N_FEATURES, "window": WINDOW,
            ...
            "best_val_f1m": best_f1m, "preprocessing": "v3_scada19_19dim"}, MODEL_OUT)
...
meta = {
    "model": f"TCN v4 +SE on v3 data",
    "tag": TAG, "preprocessing": "v3_scada19_19dim", "window": WINDOW,
...
lines = [
    "=" * 72, f"EVALUATION REPORT — TCN v4 +SE on v3 (19-dim SCADA row-level, no window agg)",
    "=" * 72,
    "Preprocessing: v3 (19 row-level features, NO window aggregates, NO one-hot)",
    f"Architecture: TCN v4 +SE (3 blocks, dilations=[1,2,4], channels=64, SE r=8)",
...
log(f"   params: {n_params:,}  |  features: {N_FEATURES}  |  train time: {train_time:.1f}s")
log("=" * 60)