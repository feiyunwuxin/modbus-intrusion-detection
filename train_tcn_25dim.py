#!/usr/bin/env python3
"""25 维 = idx 0-24 (去掉 idx 25 length_nunique_w + idx 26 unusual_count_w)

警告: 这 2 个特征在 LOO 中都是 drop-hurts:
  - length_nunique_w (25): DeltaF1m = -0.0142 (drop 后 F1m 下降)
  - unusual_count_w  (26): DeltaF1m = -0.0183 (drop 后 F1m 下降)

LOO 加总预测: DeltaF1m <= -0.0325 (期望 F1m <= 0.794)
"""
...
KEEP_25 = list(range(25))  # idx 0..24
DROP = {25: "length_nunique_w", 26: "unusual_count_w"}
...
print(f"  LOO 加总预测:        DeltaF1m <= -0.0325 (2 个 drop-hurts 特征)")
print(f"  LOO 预测满足 (mean)? {'YES (下降)' if mean_f1m - r27['test_macro_f1'] <= -0.025 else 'NO (降得不够/反而升)'}")
print(f"  LOO 预测满足 (seed=42)? {'YES (下降)' if results[0]['test_macro_f1'] - r27['test_macro_f1'] <= -0.025 else 'NO'}")

out_json = os.path.join(BASE, "train_tcn_25dim_results.json")
