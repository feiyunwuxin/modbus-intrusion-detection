#!/usr/bin/env python3
"""25 维 = 27 - {deadband (idx 6), crc_mean_w (idx 20)} retrain 验证

= 26 (-deadband) + 去 crc_mean_w (idx 20)
LOO 单点预测:
  - deadband drop: DeltaF1m = +0.0090
  - crc_mean_w drop: DeltaF1m = +0.0040
  - LOO 加总预测: DeltaF1m >= +0.0130
期望 F1m >= 0.8268 + 0.013 = 0.840
"""
...
# 25 维 = 27 - {6, 20}
KEEP_25 = [i for i in range(27) if i not in (6, 20)]
DROP = {6: "deadband", 20: "crc_mean_w"}
...
print(f"=== TCN+SE 25-dim (-deadband,-crc_mean_w) Retrain ===")
print(f"去除特征: {DROP}")
print(f"保留特征 ({len(KEEP_25)} dim)")
...
print("\n=== 25-dim (-deadband,-crc_mean_w) 5-seed ===")
results = []
...
print("\n" + "="*80)
print("=== 25-dim (-deadband,-crc_mean_w) 5-seed 汇总 ===")
print("="*80)
...
print(f"  LOO 加总预测:        DeltaF1m >= +0.0130")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - r27['test_macro_f1'] >= 0.013 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - r27['test_macro_f1'] >= 0.013 else 'NO FAIL'}")

out_json = os.path.join(BASE, "train_tcn_25dim_dc_results.json")
