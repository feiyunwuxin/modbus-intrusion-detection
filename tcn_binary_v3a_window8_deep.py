#!/usr/bin/env python3
"""
TCN v3a — ABLATION: deeper only (WINDOW=8, BLOCKS=6).
Compared to v2: same window=8, but BLOCKS 3→6, DILATIONS [1,2,4]→[1,2,4,8,16,32].

This isolates the contribution of network DEPTH.
"""
...
# Hyperparameters
SEED        = 42
WINDOW      = 8                                # 2 Modbus cycles (SAME as v2)
EPOCHS      = 35
BATCH       = 512
LR          = 5e-4
WD          = 1e-5
...
lines = [
    ...
    "=" * 72,
    f"Ablation: deeper only (vs v2)",
    f"Window: {WINDOW} (SAME as v2)  |  Blocks: {N_BLOCKS} (v2=3)  |  Dilations: {DILATIONS}",
