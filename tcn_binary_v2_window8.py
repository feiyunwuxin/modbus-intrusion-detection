#!/usr/bin/env python3
"""
TCN (Temporal Convolutional Network) binary classifier on the 17-feature v2 dataset,
sliding window=8.

TCN = stacked dilated 1D convolutions with residual connections:
- Dilations [1, 2, 4, ...] exponentially grow the receptive field
- Residual connections stabilize training of deep stacks
- Causal (or non-causal) convolutions model temporal dependencies
- Parallelizable (no recurrence → faster than LSTM/GRU on GPU; similar on CPU)
"""
...
MODEL_OUT = os.path.join(BASE, "model_tcn_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_tcn_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_tcn_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_tcn_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_tcn_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_tcn_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_tcn_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_tcn_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_tcn_binary_v2_window8.png")
...
CHANNELS    = 64
KERNEL_SIZE = 3
DILATIONS   = [1, 2, 4]               # receptive field after 3 blocks: 1+2*sum(d_i*(k-1)) = 29
...
log(f"   final TCN input shape: {X_train_cnn.shape}  → Conv1D(F={N_FEATURES}, W={WINDOW})")
...
class TCNBlock(nn.Module):
    """One TCN residual block: 2 dilated conv layers + BN + ReLU + Dropout + residual."""
    def __init__(self, in_ch, out_ch, kernel_size, dilation, dropout):
        super().__init__()
        pad = (kernel_size - 1) * dilation // 2   # same-length output
        self.conv1 = nn.Conv1d(in_ch, out_ch, kernel_size, padding=pad, dilation=dilation)
        self.bn1   = nn.BatchNorm1d(out_ch)
        self.conv2 = nn.Conv1d(out_ch, out_ch, kernel_size, padding=pad, dilation=dilation)
        self.bn2   = nn.BatchNorm1d(out_ch)
        self.drop  = nn.Dropout(dropout)
        # 1x1 conv to match residual channels
...
# Compute effective receptive field for the log
...
log(f"   Effective receptive field: {rf} (window={WINDOW}, so {'covers' if rf >= WINDOW else 'DOES NOT cover'} the full window)")
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — TCN Binary (v2, sliding window=8)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: TCN with {N_BLOCKS} residual blocks, channels={CHANNELS}, kernel={KERNEL_SIZE}, dilations={DILATIONS}")
lines.append(f"Effective receptive field: {rf} (covers {rf/WINDOW:.1f}x the window)")
...
log(f"   TCN config    : {N_BLOCKS} blocks × channels={CHANNELS}, dilations={DILATIONS}, kernel={KERNEL_SIZE}")
log(f"   receptive_fld : {rf}")
