#!/usr/bin/env python3
"""24 维 新推荐配置 = 27 - {length (idx 2), press_mean_w (idx 19), crc_mean_w (idx 20)}

基于 5-seed LOO 真实排名,只有这 3 个特征是真正可去噪声:
  - length (idx 2):          DeltaF1m = +0.0051 (5-seed) <- 真正可去 (之前以为中性)
  - press_mean_w (idx 19):   DeltaF1m = +0.0035 (5-seed) <- 真正可去 (之前以为重要)
  - crc_mean_w (idx 20):     DeltaF1m = +0.0010 (5-seed) <- 仍可去

LOO 加总预测: +0.0096
期望 F1m >= 0.8205 + 0.010 = 0.830
...
# 24 维 = 27 - {2, 19, 20}
KEEP_24 = [i for i in range(27) if i not in (2, 19, 20)]
DROP = {2: "length", 19: "press_mean_w", 20: "crc_mean_w"}
...
print(f"=== TCN+SE 24-dim (NEW: drop idx 2/19/20) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
...
print("\n" + "="*80)
print("=== 24-dim (-2,-19,-20) 5-seed 汇总 ===")
print("="*80)
...
print(f"  LOO 加总预测:        DeltaF1m >= +0.0096")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0096 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0096 else 'NO FAIL'}")
...
out_json = os.path.join(BASE, "train_tcn_24dim_new_results.json")
