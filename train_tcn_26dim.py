#!/usr/bin/env python3
"""26 维 = 27 维 - {deadband (idx 6)} retrain 验证

LOO 单点预测: DeltaF1m = +0.0090
期望 F1m >= 0.8268 + 0.009 = 0.8358
"""
...
KEEP_26 = [i for i in range(27) if i != 6]  # 27 - {6 deadband}
...
print(f"=== TCN+SE 26-dim (drop deadband only) Retrain ===")
print(f"去除 idx=6: deadband")
print(f"保留 ({len(KEEP_26)} dim)")
...
print(f"  26-dim mean:        F1m = {mean_f1m:.4f}  DeltaF1m = {mean_f1m - r27['test_macro_f1']:+.4f}")
print(f"  26-dim seed=42:     F1m = {results[0]['test_macro_f1']:.4f}  DeltaF1m = {results[0]['test_macro_f1'] - r27['test_macro_f1']:+.4f}")
print(f"  LOO 单点预测:        DeltaF1m >= +0.0090")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - r27['test_macro_f1'] >= 0.009 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - r27['test_macro_f1'] >= 0.009 else 'NO FAIL'}")

out_json = os.path.join(BASE, "train_tcn_26dim_results.json")
with open(out_json, "w") as f:
    json.dump({"baseline_27dim": r27, "results_26dim": results,
               "mean_f1m": mean_f1m, "std_f1m": std_f1m,
               "mean_pr_auc": mean_pr, "std_pr_auc": std_pr,
               "drop_idx": 6, "drop_feature": "deadband",
               "keep_idx": KEEP_26}, f, indent=2)
print(f"\nSaved: {out_json}")