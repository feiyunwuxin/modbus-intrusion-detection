#!/usr/bin/env python3
"""27 维 baseline 5-seed 公平对比 (之前只有 seed=42 单 seed)

为了让所有配置可以公平对比 5-seed mean
"""
...
print(f"=== TCN+SE 27-dim baseline 5-seed (公平对比) ===")
...
# 检查 outlier
sorted_f1m = sorted([r['test_macro_f1'] for r in results])
print(f"\nF1m 分布: min={sorted_f1m[0]:.4f}, max={sorted_f1m[-1]:.4f}, median={sorted_f1m[2]:.4f}")
outlier_seeds = [seeds[i] for i, r in enumerate(results) if r['test_macro_f1'] < mean_f1m - 2*std_f1m]
print(f"Outlier seeds (< mean-2std): {outlier_seeds}")

out_json = os.path.join(BASE, "train_tcn_27dim_5seed_results.json")
with open(out_json, "w") as f:
    json.dump({"results_27dim": results,
               "mean_f1m": mean_f1m, "std_f1m": std_f1m,
               "mean_pr_auc": mean_pr, "std_pr_auc": std_pr}, f, indent=2)
print(f"\nSaved: {out_json}")