#!/usr/bin/env python3
"""
1D-CNN binary classifier on the 17-feature v2 dataset with **sliding-window of 4 rows**
(1 Modbus cycle = 4 timesteps × 17 features).

Pipeline:
- Load npy arrays from pipeline_binary.py
- Group every 4 consecutive rows into 1 cycle → shape (n_cycles, 4, 17)
- One-hot encode function for each of the 4 timesteps
- Cycle label = 1 if any of the 4 rows has y=1
- Conv1D over the 4 timesteps (treating 4 as time, F as channels)
- Val-based threshold tuning (no test leak)
- Compare against single-row CNN v2 and LGB v2
"""
...
MODEL_OUT = os.path.join(BASE, "model_cnn_binary_v2_window.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_cnn_binary_v2_window.txt")
META_OUT  = os.path.join(BASE, "processed_meta_cnn_binary_window.json")
THR_OUT   = os.path.join(BASE, "best_threshold_cnn_window.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_cnn_binary_v2_window.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_cnn_binary_v2_window.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_cnn_binary_v2_window.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_cnn_binary_v2_window.png")
HIST_PNG  = os.path.join(BASE, "training_history_cnn_binary_v2_window.png")

# Hyperparameters
SEED        = 42
WINDOW      = 4                       # 1 Modbus cycle
EPOCHS      = 25
BATCH       = 512
LR          = 5e-4
WD          = 1e-5
...
# ─────────────────────────────────────────────
log(f"STEP 2: group every {WINDOW} rows into 1 cycle ...")
...
log(f"   final CNN input shape: {X_train_cnn.shape}  → Conv1D(F={N_FEATURES}, W={WINDOW})")
...
# Step 5 — Model
# ─────────────────────────────────────────────
class CNN1DWindow(nn.Module):
    def __init__(self, n_features, window=WINDOW, dropout=0.3):
        super().__init__()
        # Treat WINDOW as time, F as channels
...
model = CNN1DWindow(N_FEATURES, window=WINDOW, dropout=DROPOUT)
...
meta = {
    "model": "1D-CNN v2 window=4",
    "window": WINDOW,
    "n_features_per_step": int(N_FEATURES),  # 16 + 28 = 44
    "n_features_raw": 17,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — 1D-CNN Binary (v2, sliding window=4)")
lines.append("=" * 72)
