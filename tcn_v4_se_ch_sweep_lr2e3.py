#!/usr/bin/env python3
"""
TCN+SE 19-dim SCADA — Channel width sweep with LR=2e-3 (winner) locked in.

Now that LR=2e-3 is the proven winner, sweep the channel width to find
optimal capacity. LR=2e-3's stability means we can trust single-seed
results more than before.

Channel values to test:
  - 32  (very compact, ~14K params)
  - 48  (compact, ~32K params)
  - 64  ← current default (~73K params)
  - 80  (slightly wider, ~113K params)
  - 96  (wider, ~163K params)
  - 128 (very wide, ~290K params)

Single seed=42 for fair comparison, since LR=2e-3 has very low variance.
"""
...
CHANNELS_LIST = [32, 48, 64, 80, 96, 128]

# Fixed LR (proven winner) and base config
SEED         = 42
WINDOW       = 16
EPOCHS       = 40
BATCH        = 128
LR           = 2e-3     # ★ from LR sweep
WD           = 1e-5
PATIENCE     = 10
GRAD_CLIP    = 0.5
DROPOUT      = 0.3
...
t_start = time.time()
print(f"[config] sweep channels: {CHANNELS_LIST}, LR=2e-3")
print(f"[expected time] ~{len(CHANNELS_LIST) * 100 // 60} min ({len(CHANNELS_LIST)} runs × ~100s each)")
...
df.to_csv(os.path.join(BASE, "tcn_v4_se_ch_sweep_lr2e3_summary.csv"), index=False)
with open(os.path.join(BASE, "tcn_v4_se_ch_sweep_lr2e3_meta.json"), "w") as f:
    json.dump({"config": {"lr": LR, "batch": BATCH, "epochs": EPOCHS, "seed": SEED},
               "channels_tested": CHANNELS_LIST, "results": results,
               "total_time_seconds": float(total),
               "best_channels": max(results, key=lambda r: r["test_f1m"])["channels"],
               "best_test_f1m": max(r["test_f1m"] for r in results)}, f, indent=2)

print()
print("=" * 80)
print(f"  CHANNELS SWEEP @ LR=2e-3 — COMPLETE in {total:.1f}s")
print("=" * 80)
print(f"{'Channels':>10} {'Params':>10} {'TestF1m':>10} {'PR-AUC':>10} {'BestEp':>8} {'Time(s)':>8}")
print("-" * 80)
for r in results:
    print(f"{r['channels']:>10} {r['n_params']:>10,} {r['test_f1m']:>10.4f} "
          f"{r['test_pr_auc']:>10.4f} {r['best_val_epoch']:>8} {r['train_time']:>8.1f}")
print("=" * 80)
best = max(results, key=lambda r: r["test_f1m"])
print(f"\nBest: channels={best['channels']} → Test F1m = {best['test_f1m']:.4f}, "
