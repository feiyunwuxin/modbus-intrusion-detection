#!/usr/bin/env python3
"""25 维 = 27 - {length (idx 2), crc_mean_w (idx 20)} retrain 5-seed 验证

5-seed LOO 单点预测:
  - length (idx 2):     DeltaF1m = +0.0051 (正向最大)
  - crc_mean_w (idx 20): DeltaF1m = +0.0010 (正向)

LOO 加总预测: +0.0051 + 0.0010 = +0.0061
期望 F1m >= 0.8205 + 0.0061 = 0.8266
"""
...
# 25 维 = 27 - {2 length, 20 crc_mean_w}
KEEP_25 = [i for i in range(27) if i not in (2, 20)]
DROP = {2: "length", 20: "crc_mean_w"}
...
print(f"=== TCN+SE 25-dim (-length, -crc_mean_w) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
...
print("\n=== 25-dim (-length, -crc_mean_w) 5-seed ===")
results = []
for seed in SEEDS:
...
print("\n" + "="*80)
print("=== 25-dim (-length, -crc_mean_w) 5-seed 汇总 ===")
print("="*80)
...
print(f"  LOO 加总预测:        DeltaF1m >= +0.0061 (= +0.0051 + 0.0010)")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0061 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0061 else 'NO FAIL'}")
...
out_json = os.path.join(BASE, "train_tcn_25dim_lc_results.json")
