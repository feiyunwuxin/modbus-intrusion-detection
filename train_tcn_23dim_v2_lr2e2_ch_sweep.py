#!/usr/bin/env python3
"""TCN+SE 23-dim + B=64 + ep=20 × (LR=2e-2 × {ch=64, ch=128}) × 5-seed — 4D 探测

探索: 在当前最佳 23-dim + B=64 + ep=20 配置下,LR=2e-2 (5× 当前 4e-3) 在 ch=64/128 上的表现
LR=2e-2 远高于历史扫描范围 (2e-4 ~ 8e-3),可能触发不同优化景观

用法:
    python train_tcn_23dim_v2_lr2e2_ch_sweep.py --lr 2e-2 --channels 64 --seed 42
"""
...
def run_one(lr: float, channels: int, seed: int):
...
def main():
    ...
    parser.add_argument("--lr", type=float, required=True, choices=[LR_TEST])
    parser.add_argument("--channels", type=int, required=True, choices=CH_TEST)
    ...
    if args.out is None:
        ...
        args.out = os.path.join(BASE, f"train_tcn_23dim_v2_lr2e2_ch_sweep_partial_lr{lr_tag}_ch{args.channels}_s{args.seed}.json")

    print(f"[run] lr={args.lr}  ch={args.channels}  seed={args.seed}", flush=True)
    t0 = time.time()
    r = run_one(args.lr, args.channels, args.seed)
