#!/usr/bin/env python3
"""
TCN+SE 19-dim SCADA — extended training: 50 epochs + CosineAnnealingWarmRestarts.

Goal: verify whether (more epochs + better LR schedule) gives Macro-F1 boost
vs the original 35-epoch + ReduceLROnPlateau setup.

Configuration diff vs tcn_binary_v3_v4se_window16.py:
  - EPOCHS: 35 → 50
  - PATIENCE: 7 → 10
  - Scheduler: ReduceLROnPlateau → CosineAnnealingWarmRestarts(T_0=10, T_mult=2, eta_min=1e-6)
  - min_lr floor: 1e-5 → 1e-6 (for finer late-stage refinement)
"""
...
# ★ CHANGE: CosineAnnealingWarmRestarts
scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
    optimizer, T_0=10, T_mult=2, eta_min=1e-6
)
log(f"   scheduler: CosineAnnealingWarmRestarts(T_0=10, T_mult=2, eta_min=1e-6)")
log(f"   EPOCHS={EPOCHS}  PATIENCE={PATIENCE}  (was 35/7)")
...
for epoch in range(1, EPOCHS + 1):
    ...
    # Per-epoch scheduler.step() for CosineAnnealingWarmRestarts (no val needed)
    scheduler.step()
...
torch.save({
    ...
    "epochs_trained": EPOCHS, "patience": PATIENCE,
...
meta = {
    ...
    "ablation_type": "v4+SE with 50 epochs + CosineAnnealingWarmRestarts",
    "window": WINDOW,
    ...
    "history": [
        {"epoch": h[0], "loss": h[1], "val_f1m": h[2], "val_auc": h[3], "lr": h[4]}
        for h in history
    ],
}
...
log(f"   train time: {train_time:.1f}s  |  params: {n_params:,}  |  best epoch: {best_epoch}/{EPOCHS}")
log(f"   vs v3_v4se (35ep+ReduceLROnPlateau): ΔMacro-F1 = {test_thr['macro_f1'] - 0.8444:+.4f}")
log("=" * 60)