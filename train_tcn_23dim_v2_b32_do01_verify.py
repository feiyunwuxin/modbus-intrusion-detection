#!/usr/bin/env python3
"""TCN+SE 23-dim + B=32 + LR=4e-3 + ep=20 + ch=64 + do=0.1 × 5-seed — 2D 验证

测试: 把 B=64 → B=32 (2× iters/epoch 188→376),在 dropout=0.1 弱正则化下
      是否进一步提升。2 个值 vs B=64+do=0.1 baseline (F1m=0.8732) 对比

固定超参:
  - 23-dim, BATCH=32, LR=4e-3, EPOCHS=20, CHANNELS=64, dropout=0.1
  - WD=1e-5, patience=5, grad_clip=0.5
  - k=3, dilations=[1,2,4], SE r=8, W=16

用法:
    python train_tcn_23dim_v2_b32_do01_verify.py --seed 42
"""
...
def main():
    ...
    if args.out is None:
        args.out = os.path.join(BASE, f"train_tcn_23dim_v2_b32_do01_verify_partial_s{args.seed}.json")

    print(f"[run] B=32 do=0.1 seed={args.seed}", flush=True)
    t0 = time.time()
    r = run_one(args.seed)
    print(f"[done] F1m={r['test_macro_f1']:.4f}  PR-AUC={r['test_pr_auc']:.4f}  "
          f"BestEp={r['best_epoch']}/{r['actual_epochs_run']}  "
          f"Iters={r['iters_per_ep']}/ep  TrainT={r['train_time_s']:.1f}s  WallT={time.time()-t0:.1f}s", flush=True)
