#!/usr/bin/env python3
"""
TCN+SE 19-dim SCADA — verify batch size lower bound: BATCH=64.

Configuration diff vs tcn_binary_v4_se_b128_e40.py:
  - BATCH: 128 → 64
  - LR: 5e-4 → 3e-4 (smaller batch → more updates → reduce LR to compensate)

Expected:
  - Iters/epoch: 94 → 188 (2× more)
  - Total updates: 3,760 → 7,520 (2× more)
  - Train time: ~125s → ~250s
  - Test Macro-F1: 0.8655 → 0.870-0.875 (if monotonic) or 0.860-0.865 (if overfitting)
"""
...
WINDOW       = 16
EPOCHS       = 40
BATCH        = 64           # was 128  ★ KEY CHANGE
LR           = 3e-4         # was 5e-4 (compensate for 2× more updates)
WD           = 1e-5
PATIENCE     = 12           # was 10 (more patience for finer convergence)
GRAD_CLIP    = 0.5
DROPOUT      = 0.3
CLIP_VAL     = 10.0
N_BLOCKS     = 3
...
print(f"[config] BATCH={BATCH}  EPOCHS={EPOCHS}  LR={LR}  → {12014//BATCH} iters/epoch, {EPOCHS * (12014//BATCH)} total updates")
...
log(f"   iters per epoch: {len(X_train_w) // BATCH}")
...
meta = {
    ...
    "ablation_type": "v4+SE with BATCH=64, EPOCHS=40, LR=3e-4",
    "window": WINDOW,
...
log(f"   vs B=128 (ep=40):         ΔMacro-F1 = {test_thr['macro_f1'] - 0.8655:+.4f}  ΔPR-AUC = {test_thr['pr_auc'] - 0.9122:+.4f}")
log("=" * 60)