#!/usr/bin/env python3
"""26 维 = 27 - {length (idx 2)} retrain 5-seed 验证

length (idx 2) 是 5-seed LOO 单点正向贡献最大的特征:
  DeltaF1m = +0.0051 (5-seed) <- 最强正向 LOO 单点

LOO 单点预测: DeltaF1m = +0.0051
期望 F1m >= 0.8205 + 0.0051 = 0.8256
...
# 26 维 = 27 - {2 length}
KEEP_26 = [i for i in range(27) if i != 2]
DROP = {2: "length"}
...
print(f"=== TCN+SE 26-dim (-length only) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
print(f"保留特征 ({len(KEEP_26)} dim)")
...
print("\n" + "="*80)
print("=== 26-dim (-length only) 5-seed 汇总 ===")
print("="*80)
...
print(f"  26-dim mean:        F1m = {mean_f1m:.4f}  DeltaF1m = {mean_f1m - baseline_f1m_5seed:+.4f}")
print(f"  26-dim seed=42:     F1m = {results[0]['test_macro_f1']:.4f}  DeltaF1m = {results[0]['test_macro_f1'] - baseline_f1m_5seed:+.4f}")
print(f"  LOO 单点预测:        DeltaF1m >= +0.0051")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0051 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0051 else 'NO FAIL'}")
...
out_json = os.path.join(BASE, "train_tcn_26dim_length_results.json")
with open(out_json, "w") as f:
    json.dump({"results_26dim": results,
               "mean_f1m": mean_f1m, "std_f1m": std_f1m,
               "mean_pr_auc": mean_pr, "std_pr_auc": std_pr,
               "drop_features": DROP, "keep_idx": KEEP_26}, f, indent=2)
print(f"\nSaved: {out_json}")