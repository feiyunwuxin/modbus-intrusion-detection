#!/usr/bin/env python3
"""TCN+SE 23-dim + B=32 + do=0.3 × 5-seed — 5-way parallel runner
...
def main():
    ...
    for wid, seed in enumerate(SEEDS):
        wid_path = os.path.join(BASE, f"_worker_b32do03_w{wid}.py")
...
from train_tcn_23dim_v2_b32_do03_verify import run_one
...
t0 = time.time()
out = os.path.join(BASE, f"train_tcn_23dim_v2_b32_do03_verify_partial_s{{MY_SEED}}.json")
...
''')
        log_path = os.path.join(BASE, f"_worker_b32do03_w{wid}.log")
