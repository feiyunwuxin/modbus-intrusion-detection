#!/usr/bin/env python3
"""24 维 = 27 - {length (idx 2), crc_mean_w (idx 20), setpoint (idx 3)} retrain 5-seed 验证
...
LOO 加总预测: +0.0051 + 0.0010 + 0.0003 = +0.0064
期望 F1m >= 0.8205 + 0.0064 = 0.8269
"""
...
# 24 维 = 27 - {2, 20, 3}
KEEP_24 = [i for i in range(27) if i not in (2, 20, 3)]
DROP = {2: "length", 20: "crc_mean_w", 3: "setpoint"}
...
print(f"=== TCN+SE 24-dim (-length, -crc_mean_w, -setpoint) Retrain 5-seed ===")
print(f"去除特征: {DROP}")
...
print("\n=== 24-dim (-length, -crc_mean_w, -setpoint) 5-seed ===")
results = []
for seed in SEEDS:
...
print("\n" + "="*80)
print("=== 24-dim (-length, -crc_mean_w, -setpoint) 5-seed 汇总 ===")
print("="*80)
...
print(f"  LOO 加总预测:        DeltaF1m >= +0.0064 (= +0.0051 + 0.0010 + 0.0003)")
print(f"  LOO 预测满足 (mean)? {'YES OK' if mean_f1m - baseline_f1m_5seed >= 0.0064 else 'NO FAIL'}")
print(f"  LOO 预测满足 (seed=42)? {'YES OK' if results[0]['test_macro_f1'] - baseline_f1m_5seed >= 0.0064 else 'NO FAIL'}")
...
out_json = os.path.join(BASE, "train_tcn_24dim_lcs_results.json")
