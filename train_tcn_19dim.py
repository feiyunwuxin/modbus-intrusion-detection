#!/usr/bin/env python3
"""19 维 v3 retrain 验证 (历史 Macro-F1 冠军 0.8444)

19 维 v3 = 27 维前 19 个 (idx 0-18):
  - v1_raw 16: address, function, length, setpoint, gain, reset rate, deadband,
                cycle time, rate, system mode, control scheme, pump, solenoid,
                pressure measurement, crc rate, time_diff
  - v2_row 3: time_since_last_same_addr_func, is_unusual_fc, is_response
"""
...
KEEP_19 = list(range(19))  # idx 0..18
...
print(f"=== TCN+SE 19-dim v3 Retrain (历史 Macro-F1 冠军 0.8444) ===")
print(f"保留 idx 0-18 ({len(KEEP_19)} dim):")
for i, idx in enumerate(KEEP_19):
    print(f"  [{i:>2}] idx={idx:>2} {FEATURE_NAMES_27[idx]}")
print(f"\n保留: 16 v1_raw + 3 v2_row (time_since, is_unusual_fc, is_response)")
print(f"去掉: idx 19-26 (8 个 v2_window 聚合: press_mean_w, crc_mean_w, crc_max_w, cmd_count_w, resp_count_w, cmd_resp_balance_w, length_nunique_w, unusual_count_w)")
...
print("\n=== Baseline 27-dim (复现 seed=42) ===")
r27 = run_one(list(range(27)), seed=42)
print(f"  F1m={r27['test_macro_f1']:.4f}  PR-AUC={r27['test_pr_auc']:.4f}  Params={r27['n_params']:,}")

print("\n=== 19-dim v3 5-seed ===")
results = []
seeds = [42, 123, 456, 789, 1024]
for seed in seeds:
    r = run_one(KEEP_19, seed=seed)
    print(f"  seed={seed}  F1m={r['test_macro_f1']:.4f}  PR-AUC={r['test_pr_auc']:.4f}  Epoch={r['best_epoch']}  Params={r['n_params']:,}")
    results.append(r)

print("\n" + "="*80)
print("=== 19-dim v3 5-seed 汇总 ===")
print("="*80)
...
print(f"\nDelta vs 27-dim baseline (F1m={r27['test_macro_f1']:.4f}):")
print(f"  19-dim mean:        F1m = {mean_f1m:.4f}  DeltaF1m = {mean_f1m - r27['test_macro_f1']:+.4f}")
print(f"  19-dim seed=42:     F1m = {results[0]['test_macro_f1']:.4f}  DeltaF1m = {results[0]['test_macro_f1'] - r27['test_macro_f1']:+.4f}")
print(f"  历史单seed F1m:     0.8444 (单seed)")
print(f"  历史单seed PR-AUC:  0.8892 (单seed)")

out_json = os.path.join(BASE, "train_tcn_19dim_results.json")
with open(out_json, "w") as f:
    json.dump({"baseline_27dim": r27, "results_19dim": results,
               "mean_f1m": mean_f1m, "std_f1m": std_f1m,
               "mean_pr_auc": mean_pr, "std_pr_auc": std_pr,
               "keep_idx": KEEP_19}, f, indent=2)
print(f"\nSaved: {out_json}")