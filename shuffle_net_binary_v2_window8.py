#!/usr/bin/env python3
"""
1D ShuffleNet binary classifier on the 17-feature v2 dataset, sliding window=8.

ShuffleNet key ideas:
1. Pointwise GROUP convolution: instead of C_in × C_out for 1×1 conv, do (C_in/g) × (C_out/g) per group
   - This reduces params by factor of g (e.g., g=2 → 2× fewer params)
2. Channel shuffle: after group conv, shuffle channels so the NEXT layer sees all groups
   - This compensates for the loss of cross-group information flow
3. Depthwise conv for spatial mixing (cheap)
"""
...
MODEL_OUT = os.path.join(BASE, "model_shuffle_net_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_shuffle_net_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_shuffle_net_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_shuffle_net_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_shuffle_net_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_shuffle_net_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_shuffle_net_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_shuffle_net_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_shuffle_net_binary_v2_window8.png")
...
GROUPS         = 2            # g in the paper; controls pointwise group conv
# ShuffleNet units: (out_ch, stride, mid_ch)
# mid_ch is the bottleneck channel count inside the unit
UNITS = [
    (128, 1, 64),     # stride=1, mid=64
    (128, 1, 64),     # same
    (256, 1, 128),    # expand
    (256, 1, 128),    # same
]
...
log(f"   final ShuffleNet input shape: {X_train_cnn.shape}  → Conv1D(F={N_FEATURES}, W={WINDOW})")
...
# ─────────────────────────────────────────────
# Step 5 — 1D ShuffleNet
# ─────────────────────────────────────────────
def channel_shuffle1d(x, groups):
    """Shuffle channels of (B, C, T) by reordering so groups interleave."""
    B, C, T = x.shape
    if C % groups != 0:
        return x
    C_per_group = C // groups
    # Reshape to (B, groups, C_per_group, T) → permute to (B, C_per_group, groups, T) → reshape to (B, C, T)
    x = x.view(B, groups, C_per_group, T)
    x = x.transpose(1, 2).contiguous()
    return x.view(B, C, T)


class ShuffleUnit1D(nn.Module):
    """ShuffleNet v1 unit (stride=1, no downsampling)."""
    def __init__(self, in_ch, out_ch, mid_ch, groups, dropout=0.0):
        super().__init__()
        self.groups = groups
        # Pointwise group conv: in_ch → mid_ch (with groups)
        self.pw1 = nn.Conv1d(in_ch, mid_ch, 1, groups=groups, bias=False)
        self.bn1 = nn.BatchNorm1d(mid_ch)
        # Depthwise conv on mid_ch
        self.dw = nn.Conv1d(mid_ch, mid_ch, 3, padding=1, groups=mid_ch, bias=False)
        self.bn2 = nn.BatchNorm1d(mid_ch)
        # Pointwise group conv: mid_ch → out_ch
        self.pw2 = nn.Conv1d(mid_ch, out_ch, 1, groups=groups, bias=False)
        ...
        if in_ch == out_ch:
            ...
            # Need to match channels via 1x1 conv
    ...
    def forward(self, x):
        ...
        out = channel_shuffle1d(out, self.groups)
...
class ShuffleNet1D(nn.Module):
    def __init__(self, in_ch, init_channels=INIT_CHANNELS, groups=GROUPS,
        ...
        layers = []
        in_c = init_channels
        for out_c, stride, mid_c in units:
            layers.append(ShuffleUnit1D(in_c, out_c, mid_c, groups, dropout))
            in_c = out_c
...
model = ShuffleNet1D(in_ch=N_FEATURES)
n_params = sum(p.numel() for p in model.parameters())
log(f"   model params: {n_params:,}")
log(f"   architecture: Stem({N_FEATURES}→{INIT_CHANNELS}, k=3) → "
    f"{len(UNITS)} ShuffleNet units (groups={GROUPS}) → GAP → FC(32→1)")
log(f"   unit configs (out_ch, stride, mid_ch): {UNITS}")
...
meta = {
    "model": f"ShuffleNet1D v2 window={WINDOW}",
    "window": WINDOW,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — ShuffleNet1D Binary (v2, sliding window=8)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: Stem({N_FEATURES}→{INIT_CHANNELS}, k=3) → {len(UNITS)} ShuffleNet units (groups={GROUPS}) → GAP → FC(32→1)")
lines.append(f"Unit configs: {UNITS}")
...
log(f"   ShuffleNet    : stem={INIT_CHANNELS}, {len(UNITS)} units, groups={GROUPS}")
