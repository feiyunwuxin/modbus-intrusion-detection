#!/usr/bin/env python3
"""TCN+SE 23-dim + B=64 + LR=4e-3 × Epoch 扫描 — 5-way parallel runner

25 runs (5 epoch × 5 seeds). 5 workers × 5 runs each (1 seed × 5 epoch).
"""
...
def main():
    ...
    for wid, seed in enumerate(SEEDS):
        wid_path = os.path.join(BASE, f"_worker_ep23v2_w{wid}.py")
        with open(wid_path, "w", encoding="utf-8") as f:
            f.write(f'''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Worker {wid} seed={seed}, 5 epoch values"""
import os, sys, time, json
sys.path.insert(0, r"{BASE}")
from train_tcn_23dim_v2_epoch_sweep import run_one

MY_SEED = {seed}
MY_EPOCHS = {EPOCH_GRID}
WORKER_ID = {wid}
BASE = r"{BASE}"

t0 = time.time()
for ep in MY_EPOCHS:
    out = os.path.join(BASE, f"train_tcn_23dim_v2_epoch_sweep_partial_ep{{ep}}_s{{MY_SEED}}.json")
    if os.path.exists(out):
        try:
            with open(out) as f: r = json.load(f)
            print(f"[w{{WORKER_ID}}] skip ep={{ep}} s={{MY_SEED}} (cached F1m={{r['test_macro_f1']:.4f}})", flush=True)
            continue
        except: pass
    t1 = time.time()
    try:
        r = run_one(ep, MY_SEED)
        r["epoch_param"] = ep
        r["seed"] = MY_SEED
        with open(out, "w", encoding="utf-8") as f:
            json.dump(r, f, indent=2)
        print(f"[w{{WORKER_ID}}] [done] ep={{ep}} s={{MY_SEED}} "
        ...
        import traceback
        print(f"[w{{WORKER_ID}}] ERR ep={{ep}} s={{MY_SEED}}: {{e}}", flush=True)
        traceback.print_exc()

print(f"[w{{WORKER_ID}}] DONE total {{(time.time()-t0)/60:.1f}}m", flush=True)
''')
        log_path = os.path.join(BASE, f"_worker_ep23v2_w{wid}.log")
