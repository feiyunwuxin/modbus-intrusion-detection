#!/usr/bin/env python3
"""
TCN v4 — v3b base + SE (Squeeze-and-Excitation) attention.

Built on v3b (WINDOW=16, BLOCKS=3, DILATIONS=[1,2,4]) — the Pareto winner.

SE attention mechanism (Hu et al. 2018):
  1. Squeeze: GAP over time → channel descriptor (B, C)
  2. Excitation: FC(C → C/r) → ReLU → FC(C/r → C) → Sigmoid → channel weights
  3. Scale: multiply input feature map by channel weights
This learns per-channel importance dynamically — letting the network focus on
informative SCADA signals (e.g., setpoint vs command_response) per sample.

Compared to v3b: adds ~3K params (SE blocks) and ~15% training time.
Expected lift: +0.005 ~ +0.01 Macro-F1.
"""
...
DILATIONS    = [1, 2, 4]
# SE config
SE_REDUCTION = 8                 # bottleneck = CHANNELS / SE_REDUCTION = 8
...
# ─────────────────────────────────────────────
# Step 5 — SE-augmented TCN
# ─────────────────────────────────────────────
class SEBlock(nn.Module):
    """Squeeze-and-Excitation channel attention.
    GAP → FC(C → C/r) → ReLU → FC(C/r → C) → Sigmoid → x * attention
    """
    ...
    def forward(self, x):
        ...
        return x * s.unsqueeze(-1)             # (B, C, T) channel-wise scale


class TCNBlockSE(nn.Module):
    """One TCN residual block with SE attention before residual addition."""
    def __init__(self, in_ch, out_ch, kernel_size, dilation, dropout, se_reduction=SE_REDUCTION):
        ...
        self.se    = SEBlock(out_ch, reduction=se_reduction)        # ← NEW vs v3b
...
log(f"   model params: {n_params:,}  (v3b was 76,033 → +{n_params - 76033:,} from SE)")
...
meta = {
    ...
    "tag": TAG,
    "base": "v3b",
    "ablation_type": "v3b + SE attention",
    "window": WINDOW,
...
lines = [
    ...
    "=" * 72,
    f"Architecture: v3b (WINDOW=16, BLOCKS=3, DILATIONS=[1,2,4]) + SE attention",
...
log(f"   vs v3b: ΔMacro-F1 = {test_thr['macro_f1'] - 0.8326:+.4f}  ΔPR-AUC = {test_thr['pr_auc'] - 0.8999:+.4f}  ΔParams = {n_params - 76033:+,}")
log("=" * 60)
