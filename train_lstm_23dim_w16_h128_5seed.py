#!/usr/bin/env python3
"""BiLSTM classifier (hidden=128) on 23-dim × window=16, 5 seeds.

v2 scaled-up version of train_lstm_23dim_w16_5seed.py:
  - hidden 64 → 128 (params ~50K → ~157K)
...
class BiLSTMClassifier(nn.Module):
    ...
    def forward(self, x):
        # x: (B, 23, 16) — transpose to (B, 16, 23) for sequence-first LSTM
        x = x.transpose(1, 2)
        out, _ = self.lstm(x)
...
def main():
    print(f"=== BiLSTM (hidden=128) 23-dim × window=16 × 5 seeds ===")
    results = []
    for seed in SEEDS:
        print(f"\n--- seed={seed} ---")
        r = train_one_seed(BiLSTMClassifier, seed, tag=TAG, epochs=20)
    ...
    save_meta_json(results, os.path.join(BASE_PATH, f"processed_meta_{TAG}.json"),
                   model_name="BiLSTM (hidden=128)", tag=TAG,
