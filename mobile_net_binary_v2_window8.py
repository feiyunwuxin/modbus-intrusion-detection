#!/usr/bin/env python3
"""
1D MobileNet (depthwise separable convolutions) binary classifier on the 17-feature
v2 dataset, sliding window=8.

MobileNet key idea: replace standard Conv with two ops:
1. Depthwise conv: per-channel independent conv (very few params)
2. Pointwise conv: 1x1 conv to mix channels (linear projection)
This drastically reduces parameters vs standard conv → great for edge/mobile.
"""
...
MODEL_OUT = os.path.join(BASE, "model_mobile_net_binary_v2_window8.pt")
EVAL_OUT  = os.path.join(BASE, "evaluation_mobile_net_binary_v2_window8.txt")
META_OUT  = os.path.join(BASE, "processed_meta_mobile_net_binary_window8.json")
THR_OUT   = os.path.join(BASE, "best_threshold_mobile_net_window8.json")
PRED_VAL  = os.path.join(BASE, "predictions_val_mobile_net_binary_v2_window8.csv")
PRED_TEST = os.path.join(BASE, "predictions_test_mobile_net_binary_v2_window8.csv")
CM_PNG    = os.path.join(BASE, "confusion_matrix_mobile_net_binary_v2_window8.png")
PRROC_PNG = os.path.join(BASE, "pr_roc_mobile_net_binary_v2_window8.png")
HIST_PNG  = os.path.join(BASE, "training_history_mobile_net_binary_v2_window8.png")
...
CLIP_VAL    = 10.0
# MobileNet config
INIT_CHANNELS  = 64       # initial conv output channels
KERNEL_SIZE    = 3
# Depthwise-separable blocks: (out_channels, stride)
# stride=1 keeps length; we keep length=8 throughout
BLOCKS = [
    (64, 1),    # depthwise 64, pointwise 64
    (128, 1),   # depthwise 128, pointwise 128
    (128, 1),   # depthwise 128, pointwise 128
]
...
log(f"   final MobileNet input shape: {X_train_cnn.shape}  → Conv1D(F={N_FEATURES}, W={WINDOW})")
...
class DepthwiseSeparableConv1d(nn.Module):
    """MobileNet-style block: depthwise 1D conv + pointwise 1x1 conv."""
    def __init__(self, in_ch, out_ch, kernel_size, stride=1, dropout=0.0):
        super().__init__()
        pad = (kernel_size - 1) // 2
        # Depthwise: per-channel conv (groups = in_ch)
...
class MobileNet1D(nn.Module):
    def __init__(self, in_ch, init_channels=INIT_CHANNELS, kernel_size=KERNEL_SIZE,
                 blocks=BLOCKS, dropout=DROPOUT):
        super().__init__()
        # Initial standard conv (acts as "expansion" stage in MobileNet v1)
        pad = (kernel_size - 1) // 2
        ...
        for out_c, stride in blocks:
            layers.append(DepthwiseSeparableConv1d(in_c, out_c, kernel_size, stride, dropout))
            in_c = out_c
    ...
    def forward(self, x):
        # x: (B, F, T)
        x = self.stem(x)              # (B, init_channels, T)
        x = self.blocks(x)            # (B, last_block_out, T)
        x = self.gap(x).squeeze(-1)   # (B, last_block_out)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=0.3, training=self.training)
        return self.fc2(x).squeeze(-1)   # logits (B,)


model = MobileNet1D(in_ch=N_FEATURES)
n_params = sum(p.numel() for p in model.parameters())
log(f"   model params: {n_params:,}")
log(f"   architecture: Stem({N_FEATURES}→{INIT_CHANNELS}, k={KERNEL_SIZE}) "
    f"→ {len(BLOCKS)} depthwise-separable blocks "
    f"({' → '.join(f'{c}' for c, _ in BLOCKS)}) "
    f"→ GAP → FC(32→1)")

# Sanity check: confirm blocks are depthwise-separable
# Standard Conv1d would be: out_ch * in_ch * k = 128*64*3 = 24,576
# Our block is: in_ch*k (depth) + in_ch*out_ch (point) = 64*3 + 64*128 = 192 + 8192 = 8,384
# That's 3x fewer params
log(f"   param efficiency: depthwise-separable ≈ 3x fewer params than standard Conv1d")
...
meta = {
    "model": f"MobileNet1D v2 window={WINDOW}",
    "window": WINDOW,
...
lines = []
lines.append("=" * 72)
lines.append("EVALUATION REPORT — MobileNet1D Binary (v2, sliding window=8)")
lines.append("=" * 72)
lines.append(f"Window: {WINDOW} rows  |  Features per step: {N_FEATURES} (16 raw + {n_cats} one-hot function)")
lines.append(f"Architecture: Stem({N_FEATURES}→{INIT_CHANNELS}, k={KERNEL_SIZE}) → {len(BLOCKS)} depthwise-separable blocks → GAP → FC(32→1)")
lines.append(f"Block channels: {[c for c, _ in BLOCKS]}")
...
log(f"   MobileNet     : stem={INIT_CHANNELS}, {len(BLOCKS)} blocks {BLOCKS}")
