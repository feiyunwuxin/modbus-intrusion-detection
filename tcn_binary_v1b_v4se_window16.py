#!/usr/bin/env python3
"""
TCN v4 +SE on 17-dim (NO one-hot) — fair comparison companion to v3 (19-dim) and v2 (27-dim).

The 17 columns are the original v1 pipeline output (16 raw + time_diff):
  [0] address, [1] function (int, NO one-hot),
  [2] length, [3..12] setpoint..solenoid, [13] pressure measurement, [14] crc rate,
  [15] command response (binary), [16] time_diff

This is the apples-to-apples counterpart:
  - v1b_v4se_window16 → 17 Conv1D dims (no one-hot)  ← THIS SCRIPT
  - v3_v4se_window16  → 19 Conv1D dims (no one-hot)
  - v2_v4se_window16  → 27 Conv1D dims (no one-hot)
  - v4_se_window16    → 44 Conv1D dims (17 raw + 28 one-hot — reference only)

Same TCN v4+SE architecture & hyperparameters as the other three.
"""
...
TAG = "v1b_v4se_window16"
BASE = r"C:\work\Claude\Issue"

# 17-dim v1 row-level data (already preprocessed by pipeline_binary.py)
...
# TCN v4 hyperparameters — IDENTICAL to v3 (19-dim) and v2 (27-dim) for fair compare
SEED = 42
WINDOW = 16
EPOCHS = 35
BATCH = 512
...
# ─────────────────────────────────────────────
log("STEP 1: load 17-dim row-level npy ...")
...
log(f"   features per step: {N_FEATURES}  (v1b=17 no one-hot, v3=19, v2=27, v4=44 w/ one-hot)")
...
log(f"   TCN v4 +SE on {N_FEATURES}-dim input (no one-hot), params: {n_params:,}")
...
torch.save({"state_dict": best_state, "n_features": N_FEATURES, "window": WINDOW,
            ...
            "best_val_f1m": best_f1m, "preprocessing": "v1b_17dim_no_onehot"}, MODEL_OUT)
...
meta = {
    "model": f"TCN v4 +SE on v1b data (17-dim, no one-hot)",
    "tag": TAG, "preprocessing": "v1b_17dim_no_onehot", "window": WINDOW,
    ...
    "feature_order_row_level": [
        ...
        "pressure measurement", "crc rate", "command response", "time_diff",
    ],
    "metrics": {
...
lines = [
    "=" * 72, f"EVALUATION REPORT — TCN v4 +SE on v1b (17-dim, NO one-hot)",
    "=" * 72,
    "Preprocessing: v1b (17 row-level features, NO one-hot encoding of function)",
